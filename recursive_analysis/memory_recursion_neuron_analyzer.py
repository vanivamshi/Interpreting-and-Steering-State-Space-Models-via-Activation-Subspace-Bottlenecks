...
class MemoryRecursionNeuronAnalyzer:
    ...
    def analyze_memory_recursion_effects(self, input_texts: List[str], layer_indices: List[int] = None) -> Dict:
        ...
        Args:
            ...
            layer_indices: List of layer indices to analyze (default: all layers in model)
        ...
        if layer_indices is None:
            ...
            num_layers = len([m for m in self.model.modules() if 'MambaMixer' in str(type(m))])
            if num_layers == 0:
                # Fallback: try to get from config
                try:
                    num_layers = self.model.config.num_hidden_layers
                except:
                    num_layers = 24  # Default for mamba-130m-hf
            layer_indices = list(range(num_layers))  # Analyze all layers
        ...
        for i, text in enumerate(input_texts):
            ...
            for method in neuron_methods:
                ...
                for layer_idx in layer_indices:
                    if neurons is not None and layer_idx in neurons:
    ...
    def _plot_neuron_activation_patterns(self, save_dir: str):
        ...
        fig.suptitle('Neuron Activation Patterns Across Layers', fontsize=30, fontweight='bold')
            ...
            for method in ['attention_weighted', 'gradient_guided', 'rollout']:
                if method in text_data['neurons']:
                    ...
                    for layer_idx, layer_neurons in neurons.items():
                        if layer_neurons is not None and isinstance(layer_neurons, dict) and 'neuron_activations' in layer_neurons:
        ...
        if layer_means:
            ...
            axes[0, 0].set_title('Mean Neuron Activation Across Layers', fontsize=27, fontweight='bold')
            axes[0, 0].set_xlabel('Layer Index', fontsize=27, fontweight='bold')
            axes[0, 0].set_ylabel('Mean Activation', fontsize=27, fontweight='bold')
            axes[0, 0].tick_params(axis='both', which='major', labelsize=24)
            axes[0, 0].tick_params(axis='both', which='minor', labelsize=21)
        ...
        if layer_vars:
            ...
            axes[0, 1].set_title('Neuron Activation Variance Across Layers', fontsize=27, fontweight='bold')
            axes[0, 1].set_xlabel('Layer Index', fontsize=27, fontweight='bold')
            axes[0, 1].set_ylabel('Activation Variance', fontsize=27, fontweight='bold')
            axes[0, 1].tick_params(axis='both', which='major', labelsize=24)
            axes[0, 1].tick_params(axis='both', which='minor', labelsize=21)
        ...
        if top_neurons_data:
            ...
            axes[1, 0].set_title('Most Active Neurons Across Layers', fontsize=27, fontweight='bold')
            axes[1, 0].set_xlabel('Neuron Rank', fontsize=27, fontweight='bold')
            axes[1, 0].set_ylabel('Frequency', fontsize=27, fontweight='bold')
            axes[1, 0].tick_params(axis='both', which='major', labelsize=24)
            axes[1, 0].tick_params(axis='both', which='minor', labelsize=21)
            axes[1, 0].legend(fontsize=24, prop={'weight': 'bold'})
        ...
        if method_comparison:
            ...
            axes[1, 1].set_title('Neuron Activation by Method', fontsize=27, fontweight='bold')
            axes[1, 1].set_xlabel('Method', fontsize=27, fontweight='bold')
            axes[1, 1].set_ylabel('Mean Activation', fontsize=27, fontweight='bold')
            axes[1, 1].tick_params(axis='both', which='major', labelsize=24)
            axes[1, 1].tick_params(axis='both', which='minor', labelsize=21)
    ...
    def _plot_memory_effects(self, save_dir: str):
        ...
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle('Memory Effects on Neuron Behavior', fontsize=28, fontweight='bold')
        ...
        if layer_delta_magnitudes:
            ...
            axes[0, 0].errorbar(layers, means, yerr=stds, marker='o', capsize=5)
            axes[0, 0].set_title('Delta Magnitude Across Layers', fontsize=25, fontweight='bold')
            axes[0, 0].set_xlabel('Layer Index', fontsize=25, fontweight='bold')
            axes[0, 0].set_ylabel('Delta Magnitude', fontsize=25, fontweight='bold')
            axes[0, 0].tick_params(axis='both', which='major', labelsize=22)
            axes[0, 0].tick_params(axis='both', which='minor', labelsize=19)
            axes[0, 0].grid(True, alpha=0.3)
        ...
        if layer_memory_consistency:
            ...
            axes[0, 1].set_title('Memory Decay Across Layers', fontsize=25, fontweight='bold')
            axes[0, 1].set_xlabel('Layer Index', fontsize=25, fontweight='bold')
            axes[0, 1].set_ylabel('Memory Decay', fontsize=25, fontweight='bold')
            axes[0, 1].tick_params(axis='both', which='major', labelsize=22)
            axes[0, 1].tick_params(axis='both', which='minor', labelsize=19)
            axes[0, 1].grid(True, alpha=0.3)
        ...
        if layer_temporal_variance:
            ...
            axes[1, 0].set_title('Temporal Variance Across Layers', fontsize=25, fontweight='bold')
            axes[1, 0].set_xlabel('Layer Index', fontsize=25, fontweight='bold')
            axes[1, 0].set_ylabel('Temporal Variance', fontsize=25, fontweight='bold')
            axes[1, 0].tick_params(axis='both', which='major', labelsize=22)
            axes[1, 0].tick_params(axis='both', which='minor', labelsize=19)
            axes[1, 0].grid(True, alpha=0.3)
        ...
        if layer_memory_persistence:
            ...
            axes[1, 1].set_title('Memory Persistence Across Layers', fontsize=25, fontweight='bold')
            axes[1, 1].set_xlabel('Layer Index', fontsize=25, fontweight='bold')
            axes[1, 1].set_ylabel('Memory Persistence', fontsize=25, fontweight='bold')
            axes[1, 1].tick_params(axis='both', which='major', labelsize=22)
            axes[1, 1].tick_params(axis='both', which='minor', labelsize=19)
            axes[1, 1].grid(True, alpha=0.3)
    ...
    def _plot_recursive_patterns(self, save_dir: str):
        ...
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle('Recursive Patterns in Neuron Behavior', fontsize=30, fontweight='bold')
        ...
        if layer_autocorrelations:
            ...
            for i, (layer_idx, lag_data) in enumerate(list(layer_autocorrelations.items())[:4]):
                if i < 4:
                    ...
                    axes[row, col].set_title(f'Temporal Autocorrelation - Layer {layer_idx}', fontsize=27, fontweight='bold')
                    axes[row, col].set_xlabel('Lag', fontsize=27, fontweight='bold')
                    axes[row, col].set_ylabel('Autocorrelation', fontsize=27, fontweight='bold')
                    axes[row, col].grid(True, alpha=0.3)
                    axes[row, col].legend(fontsize=24, prop={'weight': 'bold'})
    ...
    def _plot_cross_layer_correlations(self, save_dir: str):
        ...
        if correlations:
            # For many layers, show a summary view with selected pairs
            num_pairs = len(correlations)
            if num_pairs > 10:
                # Show first, middle, and last pairs, plus evenly spaced ones
                selected_indices = [0] + list(range(num_pairs // 4, num_pairs, num_pairs // 4))[:3] + [num_pairs - 1]
                selected_pairs = [(k, v) for i, (k, v) in enumerate(correlations.items()) if i in selected_indices]
                title_suffix = f" (showing {len(selected_pairs)} of {num_pairs} pairs)"
            else:
                selected_pairs = list(correlations.items())
                title_suffix = ""
            
            fig, axes = plt.subplots(1, len(selected_pairs), figsize=(4.5*len(selected_pairs), 4.5))
            if len(selected_pairs) == 1:
                axes = [axes]
            
            fig.suptitle(f'Cross-Layer Correlation Heatmaps{title_suffix}', fontsize=18, fontweight='bold')
            
            for i, (pair_key, corr_data) in enumerate(selected_pairs):
                if i < len(axes):
                    ...
                    mean_corr = corr_data.get('mean_correlation', corr_matrix.mean())
                    if isinstance(mean_corr, torch.Tensor):
                        mean_corr = mean_corr.item()
                    
                    im = axes[i].imshow(corr_matrix, cmap='RdBu_r', vmin=-1, vmax=1)
                    axes[i].set_title(f'{pair_key}\nmean: {mean_corr:.4f}', fontsize=18, fontweight='bold')
                    axes[i].set_xlabel('Neuron Index\n(Layer J)', fontsize=18, fontweight='bold')
                    axes[i].set_ylabel('Neuron Index\n(Layer I)', fontsize=18, fontweight='bold')
    ...
    def _plot_ssm_neuron_correlations(self, save_dir: str):
        ...
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle('SSM-Neuron Correlation Analysis', fontsize=30, fontweight='bold')
        ...
        if layer_spectral_radius:
            ...
            axes[0, 0].set_title('Spectral Radius Across Layers', fontsize=27, fontweight='bold')
            axes[0, 0].set_xlabel('Layer Index', fontsize=27, fontweight='bold')
            axes[0, 0].set_ylabel('Spectral Radius', fontsize=27, fontweight='bold')
            axes[0, 0].tick_params(axis='both', which='major', labelsize=24)
            axes[0, 0].tick_params(axis='both', which='minor', labelsize=21)
            axes[0, 0].grid(True, alpha=0.3)
        ...
        if layer_stability:
            ...
            axes[0, 1].set_title('Stability Margin Across Layers', fontsize=27, fontweight='bold')
            axes[0, 1].set_xlabel('Layer Index', fontsize=27, fontweight='bold')
            axes[0, 1].set_ylabel('Stability Margin', fontsize=27, fontweight='bold')
            axes[0, 1].tick_params(axis='both', which='major', labelsize=24)
            axes[0, 1].tick_params(axis='both', which='minor', labelsize=21)
            axes[0, 1].grid(True, alpha=0.3)
        ...
        if layer_state_magnitude:
            ...
            axes[1, 0].set_title('Hidden State Magnitude Across Layers', fontsize=27, fontweight='bold')
            axes[1, 0].set_xlabel('Layer Index', fontsize=27, fontweight='bold')
            axes[1, 0].set_ylabel('State Magnitude', fontsize=27, fontweight='bold')
            axes[1, 0].tick_params(axis='both', which='major', labelsize=24)
            axes[1, 0].tick_params(axis='both', which='minor', labelsize=21)
            axes[1, 0].grid(True, alpha=0.3)
        ...
        if layer_state_variation:
            ...
            axes[1, 1].set_title('Temporal State Variation Across Layers', fontsize=27, fontweight='bold')
            axes[1, 1].set_xlabel('Layer Index', fontsize=27, fontweight='bold')
            axes[1, 1].set_ylabel('Temporal Variance', fontsize=27, fontweight='bold')
            axes[1, 1].tick_params(axis='both', which='major', labelsize=24)
            axes[1, 1].tick_params(axis='both', which='minor', labelsize=21)
            axes[1, 1].grid(True, alpha=0.3)
...
def demonstrate_memory_recursion_analysis():
    ...
    try:
        ...
        # Extract text samples (first 4 samples for analysis)
        test_texts = []
