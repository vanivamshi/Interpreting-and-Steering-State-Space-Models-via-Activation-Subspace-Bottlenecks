...
class MambaAttentionNeurons:
    ...
    def extract_attention_vectors(self, inputs, layer_indices: Optional[List[int]] = None):
        ...
        if layer_indices is None:
            layer_indices = list(range(len(self.model.backbone.layers)))
            ...
            if layer_idx < len(self.model.backbone.layers):
                ...
                if hasattr(layer, 'mixer'):
                    ...
                    if hasattr(mixer, 'attn_matrix_a') and hasattr(mixer, 'attn_matrix_b'):
                        ...
                        # Compute attention matrices from hidden states (fallback for standard Mamba)
                        print(f"Computing attention matrices from hidden states for layer {layer_idx}")
                        attention_matrices = self._compute_attention_from_hidden_states(inputs, layer_idx)
                        if attention_matrices is not None:
                            attention_data[layer_idx] = attention_matrices
    ...
    def _compute_attention_from_hidden_states(self, inputs, layer_idx):
        """Compute attention matrices from hidden states using cosine similarity."""
        try:
            ...
            with torch.no_grad():
                ...
                # Register hook on the mixer
                layer = self.model.backbone.layers[layer_idx]
                mixer = layer.mixer
                hook = mixer.register_forward_hook(hook_fn)
                ...
                hidden_state = hidden_states[0]  # Shape: [batch, seq_len, hidden_dim]
                
                # Compute attention matrix using cosine similarity
                batch_size, seq_len, hidden_dim = hidden_state.shape
                
                # Normalize hidden states
                hidden_norm = torch.nn.functional.normalize(hidden_state, p=2, dim=-1)
                
                # Compute cosine similarity matrix
                attention_matrix = torch.bmm(hidden_norm, hidden_norm.transpose(1, 2))
                
                # Apply softmax to get attention weights
                attention_matrix = torch.softmax(attention_matrix, dim=-1)
                
                # Create attention vectors (average across sequence dimension)
                attention_vectors = attention_matrix.mean(dim=1)  # [batch, seq_len]
                
                return {
                    'attention_matrix': attention_matrix,
                    'attention_vectors': attention_vectors,
                    'attn_matrix_a': attention_matrix,  # Use same matrix for both
                    'attn_matrix_b': attention_matrix
                }
                
        except Exception as e:
            print(f"Error computing attention from hidden states: {e}")
            return None
    ...
    def _create_rollout_neurons(self, layer_data: dict):
        ...
        # Handle different attention vector shapes (Mamba vs Transformer)
        if attention_vectors.dim() == 4:
            # Transformer-style: (batch_size, num_heads, seq_len, seq_len)
            ...
            # Mamba-style: (batch_size, seq_len, hidden_dim) or (batch_size, seq_len, seq_len)
            batch_size, seq_len, hidden_or_seq = attention_vectors.shape
            
            if hidden_or_seq == seq_len:
                # It's an attention matrix (seq_len, seq_len)
                eye = torch.eye(seq_len).expand(batch_size, seq_len, seq_len).to(attention_vectors.device)
                attention_with_residual = attention_vectors + eye
                normalized_attention = attention_with_residual / attention_with_residual.sum(dim=-1, keepdim=True)
                neurons = {
                    'rollout_attention': normalized_attention,
                    'neuron_activations': normalized_attention.mean(dim=-1),  # Average across sequence
                    'neuron_importance': normalized_attention.std(dim=-1)
                }
            else:
                # It's hidden states, use as is
                neurons = {
                    'rollout_attention': attention_vectors,
                    'neuron_activations': attention_vectors.mean(dim=1),  # Average across sequence
                    'neuron_importance': attention_vectors.std(dim=1)
                }
        elif attention_vectors.dim() == 2:
            # Simple 2D: (seq_len, hidden_dim) or (seq_len, seq_len)
            seq_len, hidden_or_seq = attention_vectors.shape
            if hidden_or_seq == seq_len:
                eye = torch.eye(seq_len).to(attention_vectors.device)
                attention_with_residual = attention_vectors + eye
                normalized_attention = attention_with_residual / attention_with_residual.sum(dim=-1, keepdim=True)
                neurons = {
                    'rollout_attention': normalized_attention,
                    'neuron_activations': normalized_attention.mean(dim=-1),
                    'neuron_importance': normalized_attention.std(dim=-1)
                }
            else:
                neurons = {
                    'rollout_attention': attention_vectors,
                    'neuron_activations': attention_vectors.mean(dim=0),
                    'neuron_importance': attention_vectors.std(dim=0)
                }
        else:
            # Fallback: use as is
            neurons = {
                'rollout_attention': attention_vectors,
                'neuron_activations': attention_vectors.flatten(),
                'neuron_importance': attention_vectors.flatten()
            }
    ...
    def analyze_neuron_behavior(self, mamba_neurons: dict, layer_idx: int = 0):
        ...
        # Check if neurons is None or not a dict
        if neurons is None or not isinstance(neurons, dict):
            return {
                'layer_idx': layer_idx,
                'num_neurons': 0,
                'mean_activation': 0,
