...
# Steered Mamba analysis (Cluster 9 neurons amplified)
python mamba_mechanistic_analysis.py --model state-spaces/mamba-130m-hf --layer 0 --samples 100 --steeredmamba

# Stable Mamba analysis (Enhanced architecture with multi-gate and multi-timescale)
...
from stable_mamba_layer import identify_stable_mamba_circuits, add_stable_mamba_activation_hooks
...
from stable_mamba_layer import attach_stable_mamba_layers
...
from create_steered_model import load_steered_model, SteeredMambaModel
...
class SparseAttention(nn.Module):
    """Sparse attention mechanism for StableMambaLayer"""
...
class StableMambaLayer(nn.Module):
    """
    FIXED StableMamba Layer with proper gradient flow
    """
...
def test_stable_mamba_layer():
    """Test function to verify StableMambaLayer implementation works correctly"""
    logger.info("Testing StableMambaLayer implementation...")
    ...
    # Create StableMambaLayer
    stable_mamba_layer = StableMambaLayer(d_model=d_model, n_gates=n_gates, d_state=d_state)
    ...
    try:
        ...
        # Check that gradients exist for all parameters
        for name, param in stable_mamba_layer.named_parameters():
            assert param.grad is not None, f"Gradient is None for parameter {name}"
            assert not torch.isnan(param.grad).any(), f"NaN gradient for parameter {name}"
        
        logger.info("✅ StableMambaLayer test passed!")
        logger.info(f"   - Input shape: {x.shape}")
        logger.info(f"   - Output shape: {output.shape}")
        logger.info(f"   - Gradient flow: OK")
        logger.info(f"   - Number of parameters: {sum(p.numel() for p in stable_mamba_layer.parameters())}")
        logger.info(f"   - Gate weights: {stable_mamba_layer.gate_weights.data}")
        logger.info(f"   - Timescale weights: {stable_mamba_layer.timescale_weights.data}")
        
        return True
        
    except Exception as e:
        logger.error(f"❌ StableMambaLayer test failed: {e}")
        return False
...
def test_all_components():
    ...
    # Test StableMambaLayer
    stable_mamba_result = test_stable_mamba_layer()
    results.append(("StableMambaLayer", stable_mamba_result))
...
class MambaMechanisticAnalyzer:
    ...
    def __init__(self, config):
        ...
        # Steered mamba steering instance
        self.steering = None
        self._steered_layers = set()  # Track which layers have steering applied
        self.is_steered = False  # Flag to track if we're using steered model
        self.is_stable_mamba = False  # Flag to track if we're using stable mamba
        self.use_stable_mamba = False  # Flag to indicate stable mamba should be used
        
        # Bind StableMamba circuit analysis functions as methods
        self.identify_stable_mamba_circuits = identify_stable_mamba_circuits
        self.add_stable_mamba_activation_hooks = add_stable_mamba_activation_hooks
    ...
    def setup(self, enable_steering: bool = None, use_stable_mamba: bool = False):
        """
        Step 0: Setup reproducible environment and instrumentation.
        
        Args:
            enable_steering: If True, enable steering. If False, disable. If None, use config.
            use_stable_mamba: If True, use stable mamba architecture instead of regular mamba.
        """
        ...
        # Store stable mamba flag
        self.use_stable_mamba = use_stable_mamba
        
        # Load model and tokenizer
        self.model, self.tokenizer = self._load_model_and_tokenizer()
        
        # Attach stable mamba layers if requested
        if use_stable_mamba:
            logger.info("🔧 Attaching Stable Mamba layers...")
            try:
                num_added = attach_stable_mamba_layers(self.model)
                if num_added > 0:
                    logger.info(f"✅ Attached Stable Mamba layers to {num_added} layers")
                    self.is_stable_mamba = True
                else:
                    logger.warning("⚠️ Could not attach Stable Mamba layers")
                    self.is_stable_mamba = False
            except Exception as e:
                logger.warning(f"⚠️ Failed to attach Stable Mamba layers: {e}")
                self.is_stable_mamba = False
        else:
            self.is_stable_mamba = False
        
        # Determine if steering should be enabled
        if enable_steering is None:
            # Check config for mode flags
            if getattr(self.config, 'use_steered_mamba', False):
                enable_steering = True
            elif getattr(self.config, 'use_stable_mamba', False):
                enable_steering = False  # Stable mamba doesn't use steering
            else:
                enable_steering = False  # Default: regular mamba
        
        if enable_steering:
            # Apply steered mamba to all layers used in analysis
            if self.steering is None:
                logger.error("❌ CRITICAL ERROR: Steering is None! Cannot proceed with steered mamba analysis.")
                raise RuntimeError("Steering must be initialized for steered mamba analysis. Check model loading.")
            
            # Get default layer indices or from config
            layer_indices = getattr(self.config, 'layer_indices', [0, 6, 12, 18])
            if hasattr(self.config, 'layer_idx'):
                layer_indices = [self.config.layer_idx] if self.config.layer_idx is not None else layer_indices
            
            # Apply steering to all specified layers
            steering_strength = getattr(self.config, 'steering_strength', 5.0)
            logger.info(f"🎯 Setting up steered mamba for layers {layer_indices} with strength {steering_strength}x")
            self.steering.apply_multi_layer_steering(layer_indices, strength=steering_strength)
            logger.info(f"✅ Applied steered mamba to layers {layer_indices} with strength {steering_strength}x")
            logger.info(f"   Steering hooks registered: {len(self.steering.hooks)} hooks")
            self.is_steered = True  # Ensure flag is set
        else:
            # Regular Mamba (no steering)
            logger.info("📊 Running REGULAR MAMBA analysis (no steering)")
            self.is_steered = False
            if self.steering is not None:
                # Remove any existing steering hooks
                self.steering.remove_steering()
                logger.info("   Removed any existing steering hooks")
            
            # Log regular Mamba status
            logger.info("=" * 80)
            print("=" * 80)
            logger.info("📊📊📊 REGULAR MAMBA (NO STEERING) 📊📊📊")
            print("📊📊📊 REGULAR MAMBA (NO STEERING) 📊📊📊")
            logger.info("   Output files will NOT have 'steered_' prefix")
            print("   Output files will NOT have 'steered_' prefix")
            logger.info("=" * 80)
            print("=" * 80)
            
            # Save regular Mamba status
            if self.experiment_logger:
                self.experiment_logger.save_results({
                    'steering_status': 'INACTIVE',
                    'is_steered': False,
                    'note': 'These results are from REGULAR MAMBA (no steering applied)'
                }, "steering_status.json")
        
        if enable_steering:
            # Log steering status - CRITICAL MESSAGE
            # Use both regular logger AND print to ensure visibility
            steering_msg = f"🎯🎯🎯 STEERED MAMBA ACTIVE: {len(self.steering.hooks)} hooks registered 🎯🎯🎯"
            logger.info("=" * 80)
            print("=" * 80)  # Also print for visibility
            logger.info(steering_msg)
            print(steering_msg)  # Also print for visibility
            logger.info(f"   All output files will be prefixed with 'steered_'")
            print(f"   All output files will be prefixed with 'steered_'")
            logger.info(f"   Steering layers: {layer_indices}")
            print(f"   Steering layers: {layer_indices}")
            logger.info(f"   Steering strength: {steering_strength}x")
            print(f"   Steering strength: {steering_strength}x")
            logger.info("=" * 80)
            print("=" * 80)
            
            # Also save steering status to a file for verification
            if self.experiment_logger:
                self.experiment_logger.save_results({
                    'steering_status': 'ACTIVE',
                    'num_hooks': len(self.steering.hooks),
                    'steering_layers': layer_indices,
                    'steering_strength': steering_strength,
                    'cluster9_neurons': self.steering.cluster9_neurons,
    ...
    def _get_filename_prefix(self):
        """Get filename prefix based on whether steering is active."""
        return "steered_" if self.is_steered else ""
    
    def _ensure_steering_for_layer(self, layer_idx: int):
        """
        Ensure steering is applied to the specified layer.
        This is called before any analysis step that uses a specific layer.
        """
        if self.steering is not None and layer_idx not in self._steered_layers:
            steering_strength = getattr(self.config, 'steering_strength', 5.0)
            self.steering.apply_steering(strength=steering_strength, layer_idx=layer_idx)
            self._steered_layers.add(layer_idx)
            logger.info(f"🎯 Applied steered mamba to layer {layer_idx} (strength: {steering_strength}x)")
        elif self.steering is None:
            logger.warning(f"⚠️ Steering is None - steered mamba not initialized!")
    ...
    def collect_activations(self, texts: List[str], layer_indices: List[int] = None) -> Dict[int, torch.Tensor]:
        ...
        # Ensure steering is applied to all layers being analyzed
        if self.steering is not None:
            logger.info(f"🎯 Ensuring steered mamba is active for layers {layer_indices}")
            for layer_idx in layer_indices:
                self._ensure_steering_for_layer(layer_idx)
            logger.info(f"✅ Steered mamba active for layers {layer_indices} ({len(self.steering.hooks)} hooks registered)")
        else:
            logger.warning("⚠️ WARNING: Steered mamba is NOT initialized - running regular Mamba (not steered)!")
        ...
        # Setup StableMamba activation hooks for the specified layers (only if StableMamba mode is enabled)
        stable_mamba_hooks_added = False
        if not hasattr(self, 'stable_mamba_activation_data'):
            self.stable_mamba_activation_data = {}
        
        # Only collect StableMamba activations if StableMamba mode is enabled
        should_collect_stable_mamba = getattr(self.config, 'use_stable_mamba', False)
        
        if should_collect_stable_mamba:
            for layer_idx in layer_indices:
                try:
                    # Initialize storage for this layer (will store activations per text)
                    self.stable_mamba_activation_data[layer_idx] = []
                    
                    # Get the layer and add hook for final StableMamba output
                    from utils import get_model_layers
                    layers = get_model_layers(self.model)
                    if layers and layer_idx < len(layers):
                        layer = layers[layer_idx]
                        if hasattr(layer, 'stable_mamba'):
                            # Hook the final output of the StableMamba module (not the wrapper)
                            # This gives us the pure StableMamba output before mixing with original
                            def make_hook(layer_idx):
                                def stable_mamba_output_hook(module, input, output):
                                    # Store the final output tensor
                                    # Handle both 2D and 3D outputs
                                    if output.dim() == 2:
                                        # [seq_len, hidden_dim] -> add batch dimension
                                        output = output.unsqueeze(0)
                                    elif output.dim() == 3:
                                        # [batch, seq_len, hidden_dim] - already correct
                                        pass
                                    else:
                                        logger.warning(f"Unexpected StableMamba output shape: {output.shape}")
                                    
                                    # Store detached tensor
                                    if layer_idx not in self.stable_mamba_activation_data:
                                        self.stable_mamba_activation_data[layer_idx] = []
                                    # Ensure it's a list (might have been overwritten as tensor elsewhere)
                                    if not isinstance(self.stable_mamba_activation_data[layer_idx], list):
                                        self.stable_mamba_activation_data[layer_idx] = []
                                    self.stable_mamba_activation_data[layer_idx].append(output.detach())
                                return stable_mamba_output_hook
                            
                            # Register hook on the StableMamba module's forward (not the wrapper)
                            layer.stable_mamba.register_forward_hook(make_hook(layer_idx))
                            logger.info(f"✅ Added StableMamba output hook for layer {layer_idx}")
                            stable_mamba_hooks_added = True
                        else:
                            logger.warning(f"Layer {layer_idx} does not have stable_mamba module")
                    else:
                        logger.warning(f"Could not get layers for StableMamba hook registration")
                except Exception as e:
                    logger.warning(f"Could not add StableMamba hooks for layer {layer_idx}: {e}")
                    import traceback
                    logger.debug(traceback.format_exc())
            
            if not stable_mamba_hooks_added:
                logger.info("ℹ️ No StableMamba modules found - will use regular Mamba activations as StableMamba baseline for comparison")
        ...
        for i, text in enumerate(texts):
            ...
            # Collect StableMamba activations using hooks (only if StableMamba mode is enabled)
            should_collect_stable_mamba = getattr(self.config, 'use_stable_mamba', False)
            if should_collect_stable_mamba:
                try:
                    # Run forward pass to trigger StableMamba hooks
                    with torch.no_grad():
                        outputs = self.model(**inputs)
                    
                    # Extract StableMamba activations from hooks (stored in self.stable_mamba_activation_data)
                    stable_mamba_data_found = False
                    if hasattr(self, 'stable_mamba_activation_data'):
                        for layer_idx in layer_indices:
                            if layer_idx in self.stable_mamba_activation_data:
                                # Get the stored activations for this text
                                stored_activations = self.stable_mamba_activation_data[layer_idx]
                                
                                # Handle both list (from hooks) and tensor (from previous processing) cases
                                if isinstance(stored_activations, list):
                                    # The hook should have stored the output for this forward pass
                                    # Get the last stored activation (most recent)
                                    if stored_activations and len(stored_activations) > 0:
                                        stable_mamba_output = stored_activations[-1]  # Get the most recent output
                                        
                                        # Ensure it's a tensor
                                        if not isinstance(stable_mamba_output, torch.Tensor):
                                            stable_mamba_output = torch.from_numpy(stable_mamba_output) if isinstance(stable_mamba_output, np.ndarray) else stable_mamba_output
                                        
                                        # Ensure it's on the correct device
                                        if stable_mamba_output.device != inputs["input_ids"].device:
                                            stable_mamba_output = stable_mamba_output.to(inputs["input_ids"].device)
                                        
                                        if layer_idx not in all_stable_mamba_activations:
                                            all_stable_mamba_activations[layer_idx] = []
                                        all_stable_mamba_activations[layer_idx].append(stable_mamba_output)
                                        logger.debug(f"Collected StableMamba activation for layer {layer_idx}: {stable_mamba_output.shape}")
                                        stable_mamba_data_found = True
                                        
                                        # Clear the stored activation for this layer to prepare for next text
                                        self.stable_mamba_activation_data[layer_idx] = []
                                elif isinstance(stored_activations, torch.Tensor):
                                    # If it's already a tensor (from previous processing), use it directly
                                    stable_mamba_output = stored_activations
                                    if stable_mamba_output.device != inputs["input_ids"].device:
                                        stable_mamba_output = stable_mamba_output.to(inputs["input_ids"].device)
                                    ...
                                    if layer_idx not in all_stable_mamba_activations:
                                        all_stable_mamba_activations[layer_idx] = []
                                    all_stable_mamba_activations[layer_idx].append(stable_mamba_output)
                                    logger.debug(f"Collected StableMamba activation (tensor) for layer {layer_idx}: {stable_mamba_output.shape}")
                                    stable_mamba_data_found = True
                                    
                                    # Reset to list for future hook calls
                                    self.stable_mamba_activation_data[layer_idx] = []
                    
                    # Fallback: Use regular Mamba activations as StableMamba baseline if no StableMamba data found
                    if not stable_mamba_data_found:
                        logger.debug("No StableMamba data found, using regular Mamba activations as StableMamba baseline")
                        for layer_idx in layer_indices:
                            if layer_idx in activations:
                                if layer_idx not in all_stable_mamba_activations:
                                    all_stable_mamba_activations[layer_idx] = []
                                all_stable_mamba_activations[layer_idx].append(activations[layer_idx])
                                logger.debug(f"Using regular Mamba activation as StableMamba baseline for layer {layer_idx}: {activations[layer_idx].shape}")
                                
                except Exception as e:
                    logger.warning(f"Could not collect StableMamba activations: {e}")
                    import traceback
                    logger.debug(traceback.format_exc())
                    # Fallback: Use regular Mamba activations as StableMamba baseline
                    logger.debug("Using regular Mamba activations as StableMamba baseline due to error")
                    for layer_idx in layer_indices:
                        if layer_idx in activations:
                            if layer_idx not in all_stable_mamba_activations:
                                all_stable_mamba_activations[layer_idx] = []
        ...
        final_activations = {}
        final_stable_mamba_activations = {}
        ...
        # Concatenate StableMamba activations across texts (only if StableMamba mode is enabled)
        should_collect_stable_mamba = getattr(self.config, 'use_stable_mamba', False)
        if should_collect_stable_mamba:
            for layer_idx, activation_list in all_stable_mamba_activations.items():
                if activation_list:
                    # Handle variable sequence lengths by flattening first
                    flattened_activations = []
                    for activation in activation_list:
                    ...
                    # Now concatenate the flattened activations
                    concatenated = torch.cat(flattened_activations, dim=0)
                    final_stable_mamba_activations[layer_idx] = concatenated
                    
                    logger.info(f"StableMamba Layer {layer_idx}: {concatenated.shape}")
        
        # Compute baseline statistics
        baseline_stats = self._compute_baseline_stats(final_activations)
        # Only compute StableMamba baseline stats if StableMamba mode is enabled
        should_collect_stable_mamba = getattr(self.config, 'use_stable_mamba', False)
        stable_mamba_baseline_stats = {}
        if should_collect_stable_mamba and final_stable_mamba_activations:
            stable_mamba_baseline_stats = self._compute_baseline_stats(final_stable_mamba_activations)
        
        # Add steering metadata to baseline stats
        if self.is_steered and self.steering is not None:
            baseline_stats['steering_metadata'] = {
                'is_steered': True,
                'steering_strength': getattr(self.config, 'steering_strength', 5.0),
                'steering_layers': list(self._steered_layers),
                'num_hooks': len(self.steering.hooks),
                'cluster9_neurons': self.steering.cluster9_neurons,
                'note': 'These results are from STEERED MAMBA (Cluster 9 neurons amplified)'
            }
            # Verify steering is working by checking if Cluster 9 neurons have higher variance
            for layer_idx in layer_indices:
                if layer_idx in final_activations:
                    activations = final_activations[layer_idx]
                    cluster9_activations = activations[:, self.steering.cluster9_neurons]
                    other_activations = activations[:, [i for i in range(activations.shape[-1]) if i not in self.steering.cluster9_neurons]]
                    
                    cluster9_std = float(cluster9_activations.std().item())
                    other_std = float(other_activations.std().item())
                    
                    if layer_idx not in baseline_stats:
                        baseline_stats[f'layer_{layer_idx}'] = {}
                    baseline_stats[f'layer_{layer_idx}']['steering_verification'] = {
                        'cluster9_neurons_std': cluster9_std,
                        'other_neurons_std': other_std,
                        'ratio': cluster9_std / other_std if other_std > 0 else 0.0,
                        'steering_applied': cluster9_std > other_std * 1.5  # Should be higher if steering is working
                    }
                    logger.info(f"🔍 Layer {layer_idx} steering verification: Cluster9 std={cluster9_std:.4f}, Other std={other_std:.4f}, Ratio={cluster9_std/other_std if other_std > 0 else 0:.2f}")
        else:
            baseline_stats['steering_metadata'] = {
                'is_steered': False,
                'note': 'These results are from REGULAR MAMBA (no steering applied)'
        ...
        # Only store StableMamba activation data if StableMamba mode is enabled
        should_collect_stable_mamba = getattr(self.config, 'use_stable_mamba', False)
        if should_collect_stable_mamba:
            self.stable_mamba_activation_data = final_stable_mamba_activations
        else:
            self.stable_mamba_activation_data = {}
        self.experiment_logger.save_activations(final_activations)
        prefix = self._get_filename_prefix()
        self.experiment_logger.save_results(baseline_stats, f"{prefix}baseline_stats.json")
        if should_collect_stable_mamba and stable_mamba_baseline_stats:
            self.experiment_logger.save_results(stable_mamba_baseline_stats, f"{prefix}stable_mamba_baseline_stats.json")
    ...
    def discover_interpretable_features(self, layer_idx: int = 0) -> Dict[str, Any]:
        ...
        # NEW: Run SAE analysis on StableMamba activations if available and StableMamba mode is enabled
        should_analyze_stable_mamba = getattr(self.config, 'use_stable_mamba', False)
        if should_analyze_stable_mamba and hasattr(self, 'stable_mamba_activation_data') and layer_idx in self.stable_mamba_activation_data:
            logger.info(f"Step 3b: Running SAE analysis on StableMamba activations for layer {layer_idx}...")
            stable_mamba_activations = self.stable_mamba_activation_data[layer_idx]
            
            stable_mamba_sae_results = {}
            for task_name, task_labels in task_labels_dict.items():
                logger.info(f"   📊 Analyzing StableMamba task: {task_name}")
                
                # Ensure labels match activations
                if len(task_labels) != stable_mamba_activations.shape[0]:
                    logger.warning(f"   ⚠️ Label size mismatch: {len(task_labels)} vs {stable_mamba_activations.shape[0]}")
                    if len(task_labels) > stable_mamba_activations.shape[0]:
                        task_labels = task_labels[:stable_mamba_activations.shape[0]]
                    else:
                        padding = np.full(stable_mamba_activations.shape[0] - len(task_labels), task_labels.mean())
                        task_labels = np.concatenate([task_labels, padding])
                
                # Normalize labels
                task_labels = (task_labels - task_labels.mean()) / (task_labels.std() + 1e-8)
                
                # Run SAE on StableMamba activations
                stable_mamba_sae_results[task_name] = run_sae_analysis(stable_mamba_activations, task_labels, sae_config)
            
            # Save StableMamba SAE results
            self.experiment_logger.save_results(stable_mamba_sae_results, f"stable_mamba_sae_results_layer_{layer_idx}.json")
            logger.info(f"✅ StableMamba SAE analysis complete for layer {layer_idx}")
        else:
            logger.warning(f"No StableMamba activation data found for layer {layer_idx}")
    ...
    def run_hypothesis_probes(self, layer_idx=0, pool='mean', task_name='magnitude'):
        ...
        # NEW: Run hypothesis probes on StableMamba activations if available and StableMamba mode is enabled
        should_analyze_stable_mamba = getattr(self.config, 'use_stable_mamba', False)
        if should_analyze_stable_mamba and hasattr(self, 'stable_mamba_activation_data') and layer_idx in self.stable_mamba_activation_data:
            logger.info(f"Step 4b: Running hypothesis probes on StableMamba activations for layer {layer_idx}...")
            stable_mamba_acts = self.stable_mamba_activation_data[layer_idx].cpu().numpy()
            
            if stable_mamba_acts.ndim == 3:
                if pool=='mean':
                    stable_mamba_acts_pooled = stable_mamba_acts.mean(axis=1)
                elif pool=='max':
                    stable_mamba_acts_pooled = stable_mamba_acts.max(axis=1)[0]
            else:
                stable_mamba_acts_pooled = stable_mamba_acts
            
            # Generate task labels for StableMamba
            if task_name == 'magnitude':
                stable_mamba_task_labels = np.linalg.norm(stable_mamba_acts_pooled, axis=1)
            elif task_name == 'pca_component':
                stable_mamba_task_labels = self._get_pca_labels(stable_mamba_acts_pooled)
            elif task_name == 'sparsity':
                stable_mamba_task_labels = np.mean(np.abs(stable_mamba_acts_pooled) < 0.01, axis=1)
            elif task_name == 'position':
                stable_mamba_task_labels = np.arange(stable_mamba_acts_pooled.shape[0]) % 50
            else:
                stable_mamba_task_labels = np.random.randn(stable_mamba_acts_pooled.shape[0])
            
            # Run probes on StableMamba
            stable_mamba_probe_results = self._run_probe_analysis(stable_mamba_acts_pooled, stable_mamba_task_labels, task_name)
            
            # Save StableMamba probe results
            self.experiment_logger.save_results(stable_mamba_probe_results, f"stable_mamba_probe_results_layer_{layer_idx}_{task_name}.json")
            logger.info(f"✅ StableMamba hypothesis probes complete for layer {layer_idx}")
        else:
            logger.warning(f"No StableMamba activation data found for layer {layer_idx}")
    ...
    def _run_probe_analysis(self, acts_pooled, task_labels, task_name):
        ...
        This is a helper method for running probes on StableMamba activations.
        """
        from sklearn.preprocessing import StandardScaler
        from sklearn.linear_model import RidgeCV
        import numpy as np
    ...
    def run_stable_mamba_analysis_with_skip_steps(self, skip_steps):
        """
        Run StableMamba analysis for the same steps as regular Mamba analysis.
        This ensures StableMamba runs steps 1-6 when steps 7-19 are skipped,
        or steps 1-9 when steps 10-19 are skipped, etc.
        """
        logger.info(f"🔄 Running StableMamba analysis with skip_steps: {skip_steps}")
        
        # Step 2: StableMamba Activation collection (if not skipped)
        if '2' not in skip_steps:
            logger.info("Step 2b: Collecting StableMamba activations...")
            if hasattr(self, 'stable_mamba_activation_data') and 0 in self.stable_mamba_activation_data:
                logger.info("✅ StableMamba activations already collected")
            else:
                logger.warning("StableMamba activations not available - skipping StableMamba analysis")
                return
        
        # Step 3: StableMamba SAE discovery (if not skipped)
        if '3' not in skip_steps:
            logger.info("Step 3b: Running SAE analysis on StableMamba activations...")
            if hasattr(self, 'stable_mamba_activation_data') and 0 in self.stable_mamba_activation_data:
                stable_mamba_acts = self.stable_mamba_activation_data[0].cpu().numpy()
                if stable_mamba_acts.ndim == 3:
                    stable_mamba_acts_pooled = stable_mamba_acts.mean(axis=1)
                else:
                    stable_mamba_acts_pooled = stable_mamba_acts
                ...
                stable_mamba_sae_results = run_sae_analysis(torch.tensor(stable_mamba_acts_pooled), stable_mamba_task_labels, sae_config)
                self.experiment_logger.save_results(stable_mamba_sae_results, f"stable_mamba_sae_results_layer_0.json")
                logger.info("✅ StableMamba SAE analysis complete!")
        
        # Step 4: StableMamba Hypothesis probes (if not skipped)
        if '4' not in skip_steps:
            logger.info("Step 4b: Running hypothesis probes on StableMamba activations...")
            if hasattr(self, 'stable_mamba_activation_data') and 0 in self.stable_mamba_activation_data:
                stable_mamba_acts = self.stable_mamba_activation_data[0].cpu().numpy()
                if stable_mamba_acts.ndim == 3:
                    stable_mamba_acts_pooled = stable_mamba_acts.mean(axis=1)
                ...
                task_labels = np.linalg.norm(stable_mamba_acts_pooled, axis=1)
                
                # Run probes on StableMamba
                stable_mamba_probe_results = self._run_probe_analysis(stable_mamba_acts_pooled, task_labels, 'magnitude')
                
                # Save StableMamba probe results
                self.experiment_logger.save_results(stable_mamba_probe_results, f"stable_mamba_probe_results_layer_0_magnitude.json")
                logger.info("✅ StableMamba hypothesis probes complete!")
        
        # Step 5: StableMamba Candidate circuit selection (if not skipped)
        if '5' not in skip_steps:
            logger.info("Step 5b: Selecting candidate circuits for StableMamba...")
            if hasattr(self, 'stable_mamba_activation_data') and 0 in self.stable_mamba_activation_data:
                # Use the same logic as regular Mamba but for StableMamba activations
                stable_mamba_circuits = self.select_candidate_circuits_stable_mamba(0)
                self.experiment_logger.save_results(stable_mamba_circuits, f"stable_mamba_candidate_circuits_layer_0.json")
                logger.info("✅ StableMamba candidate circuit selection complete!")
        
        # Step 6: StableMamba Activation patching (if not skipped)
        if '6' not in skip_steps:
            logger.info("Step 6b: Testing StableMamba circuit causality with activation patching...")
            if hasattr(self, 'stable_mamba_activation_data') and 0 in self.stable_mamba_activation_data:
                # Run activation patching for StableMamba
                stable_mamba_patching_results = self.test_circuit_causality_stable_mamba(0)
                self.experiment_logger.save_results(stable_mamba_patching_results, f"stable_mamba_patching_results_layer_0.json")
                logger.info("✅ StableMamba activation patching complete!")
        
        # Step 7: StableMamba Memory horizons analysis (if not skipped)
        if '7' not in skip_steps:
            logger.info("Step 7b: Analyzing StableMamba memory horizons...")
            if hasattr(self, 'stable_mamba_activation_data') and 0 in self.stable_mamba_activation_data:
                # Run memory horizons analysis for StableMamba
                stable_mamba_memory_horizons = self.analyze_memory_horizons_stable_mamba(0)
                self.experiment_logger.save_results(stable_mamba_memory_horizons, f"stable_mamba_memory_horizons_layer_0.json")
                logger.info("✅ StableMamba memory horizons analysis complete!")
        
        # Step 8: StableMamba Temporal causality analysis (if not skipped)
        if '8' not in skip_steps:
            logger.info("Step 8b: Analyzing StableMamba temporal causality...")
            if hasattr(self, 'stable_mamba_activation_data') and 0 in self.stable_mamba_activation_data:
                # Run temporal causality analysis for StableMamba
                stable_mamba_temporal_results = self.analyze_temporal_causality_stable_mamba(0)
                self.experiment_logger.save_results(stable_mamba_temporal_results, f"stable_mamba_temporal_results_layer_0.json")
                
                # Generate temporal dynamics visualizations (same as original Mamba)
                logger.info("Generating StableMamba temporal dynamics visualizations...")
                test_texts = [
                    "The quick brown fox jumps over the lazy dog.",
                    "Artificial intelligence is transforming industries worldwide.",
                    "Machine learning models require large amounts of training data.",
                ...
                logger.info(f"StableMamba temporal visualization saved: {viz_results['temporal_visualization']}")
                
                logger.info("✅ StableMamba temporal causality analysis complete!")
        
        # Step 9: StableMamba Causal equivalence analysis (if not skipped)
        if '9' not in skip_steps:
            logger.info("Step 9b: Running StableMamba causal equivalence analysis...")
            if hasattr(self, 'stable_mamba_activation_data') and 0 in self.stable_mamba_activation_data:
                try:
                    # Run causal equivalence analysis for StableMamba
                    stable_mamba_causal_eq_results = self.run_causal_equivalence_analysis_stable_mamba(0)
                    self.experiment_logger.save_results(stable_mamba_causal_eq_results, f"stable_mamba_causal_equivalence_layer_0.json")
                    logger.info("✅ StableMamba causal equivalence analysis complete!")
                except Exception as e:
                    logger.error(f"StableMamba causal equivalence analysis failed: {e}")
                    logger.info("Continuing with remaining steps...")
            else:
                logger.warning("No StableMamba activation data available for Step 9")
        
        # Step 10: StableMamba Dynamic universality analysis (if not skipped)
        if '10' not in skip_steps:
            logger.info("Step 10b: Running StableMamba dynamic universality analysis...")
            if hasattr(self, 'stable_mamba_activation_data') and 0 in self.stable_mamba_activation_data:
                # Run dynamic universality analysis for StableMamba
                stable_mamba_dyn_uni_results = self.run_dynamic_universality_analysis_stable_mamba(0)
                self.experiment_logger.save_results(stable_mamba_dyn_uni_results, f"stable_mamba_dynamic_universality_layer_0.json")
                logger.info("✅ StableMamba dynamic universality analysis complete!")
        
        # Step 11: StableMamba Mechanistic diagnostics (if not skipped)
        if '11' not in skip_steps:
            logger.info("Step 11b: Running StableMamba mechanistic diagnostics...")
            if hasattr(self, 'stable_mamba_activation_data') and 0 in self.stable_mamba_activation_data:
                # Run mechanistic diagnostics for StableMamba
                stable_mamba_mechanistic_results = self.run_mechanistic_diagnostics_stable_mamba(0)
                self.experiment_logger.save_results(stable_mamba_mechanistic_results, f"stable_mamba_mechanistic_diagnostics_layer_0.json")
                logger.info("✅ StableMamba mechanistic diagnostics complete!")
        
        # Step 12: StableMamba Feature superposition analysis (if not skipped)
        if '12' not in skip_steps:
            logger.info("Step 12b: Analyzing StableMamba feature superposition...")
            if hasattr(self, 'stable_mamba_activation_data') and 0 in self.stable_mamba_activation_data:
                # Run feature superposition analysis for StableMamba
                stable_mamba_superposition_results = self.analyze_feature_superposition_stable_mamba(0)
                self.experiment_logger.save_results(stable_mamba_superposition_results, f"stable_mamba_feature_superposition_layer_0.json")
                logger.info("✅ StableMamba feature superposition analysis complete!")
        
        # Step 13: StableMamba Dictionary learning (if not skipped)
        if '13' not in skip_steps:
            logger.info("Step 13b: Running StableMamba dictionary learning...")
            if hasattr(self, 'stable_mamba_activation_data') and 0 in self.stable_mamba_activation_data:
                # Run dictionary learning for StableMamba
                stable_mamba_dict_results = self.run_stable_mamba_dictionary_learning(0)
                self.experiment_logger.save_results(stable_mamba_dict_results, f"stable_mamba_dictionary_learning_layer_0.json")
                logger.info("✅ StableMamba dictionary learning complete!")
        
        # Step 14: StableMamba Scaling analysis (if not skipped)
        if '14' not in skip_steps:
            logger.info("Step 14b: Running StableMamba scaling analysis...")
            if hasattr(self, 'stable_mamba_activation_data') and 0 in self.stable_mamba_activation_data:
                # Run scaling analysis for StableMamba
                stable_mamba_scaling_results = self.compare_across_model_scales_stable_mamba()
                self.experiment_logger.save_results(stable_mamba_scaling_results, f"stable_mamba_scaling_analysis_layer_0.json")
                logger.info("✅ StableMamba scaling analysis complete!")
        
        # Step 15: StableMamba Grokking analysis (if not skipped)
        if '15' not in skip_steps:
            logger.info("Step 15b: Running StableMamba grokking analysis...")
            if hasattr(self, 'stable_mamba_activation_data') and 0 in self.stable_mamba_activation_data:
                # Run grokking analysis for StableMamba
                stable_mamba_grokking_results = self.run_grokking_analysis_stable_mamba(0)
                self.experiment_logger.save_results(stable_mamba_grokking_results, f"stable_mamba_grokking_analysis_layer_0.json")
                logger.info("✅ StableMamba grokking analysis complete!")
        
        # Step 16: StableMamba Sparse probing visualization (if not skipped)
        if '16' not in skip_steps:
            logger.info("Step 16b: Running StableMamba sparse probing visualization...")
            if hasattr(self, 'stable_mamba_activation_data') and 0 in self.stable_mamba_activation_data:
                # Run sparse probing visualization for StableMamba
                stable_mamba_probe_viz_results = self.visualize_sparse_probing_stable_mamba(0)
                self.experiment_logger.save_results(stable_mamba_probe_viz_results, f"stable_mamba_sparse_probing_layer_0.json")
                logger.info("✅ StableMamba sparse probing visualization complete!")
        
        # Step 17: StableMamba SPD analysis (if not skipped)
        if '17' not in skip_steps:
            logger.info("Step 17b: Running StableMamba Stochastic Parameter Decomposition (SPD) analysis...")
            if hasattr(self, 'stable_mamba_activation_data') and 0 in self.stable_mamba_activation_data:
                # Run SPD analysis for StableMamba
                stable_mamba_spd_results = self.run_spd_analysis_stable_mamba(0)
                self.experiment_logger.save_results(stable_mamba_spd_results, f"stable_mamba_spd_analysis_layer_0.json")
                logger.info("✅ StableMamba SPD analysis complete!")
        
        # Step 18: StableMamba APD analysis (if not skipped)
        if '18' not in skip_steps:
            logger.info("Step 18b: Running StableMamba Attribution-based Parameter Decomposition (APD) analysis...")
            if hasattr(self, 'stable_mamba_activation_data') and 0 in self.stable_mamba_activation_data:
                # Run APD analysis for StableMamba
                stable_mamba_apd_results = self.run_apd_analysis_stable_mamba(0)
                self.experiment_logger.save_results(stable_mamba_apd_results, f"stable_mamba_apd_analysis_layer_0.json")
                logger.info("✅ StableMamba APD analysis complete!")
        
        # Step 19: StableMamba Post-SPD cluster analysis (if not skipped)
        if '19' not in skip_steps:
            logger.info("Step 19b: Running StableMamba post-SPD cluster analysis...")
            if hasattr(self, 'stable_mamba_activation_data') and 0 in self.stable_mamba_activation_data:
                # Run post-SPD cluster analysis for StableMamba
                stable_mamba_post_spd_results = self.run_post_spd_analysis_stable_mamba(0)
                self.experiment_logger.save_results(stable_mamba_post_spd_results, f"stable_mamba_post_spd_analysis_layer_0.json")
                logger.info("✅ StableMamba post-SPD cluster analysis complete!")
        
        logger.info("✅ StableMamba analysis with skip_steps complete!")
    
    def select_candidate_circuits_stable_mamba(self, layer_idx: int = 0):
        """Select candidate circuits for StableMamba using StableMamba-specific logic"""
        logger.info(f"Step 5b: Selecting candidate circuits for StableMamba layer {layer_idx}...")
        
        if not hasattr(self, 'stable_mamba_activation_data') or layer_idx not in self.stable_mamba_activation_data:
            logger.warning(f"No StableMamba activation data found for layer {layer_idx}")
            return {'circuits': [], 'count': 0, 'architecture': 'stable_mamba', 'layer_idx': layer_idx}
        
        stable_mamba_candidate_circuits = []
        
        # Check for StableMamba SAE results
        stable_mamba_sae_file = self.experiment_logger.experiment_dir / f"stable_mamba_sae_results_layer_{layer_idx}.json"
        if stable_mamba_sae_file.exists():
            logger.info(f"   Found StableMamba SAE file: {stable_mamba_sae_file}")
            with open(stable_mamba_sae_file, 'r') as f:
                stable_mamba_sae_results = json.load(f)
            
            # Handle both task-specific and flat structure
            if isinstance(stable_mamba_sae_results, dict) and any(isinstance(v, dict) for v in stable_mamba_sae_results.values()):
                # Task-specific structure (expected)
                for task_name, task_results in stable_mamba_sae_results.items():
                    if isinstance(task_results, dict) and 'top_correlated_dims' in task_results:
                        ...
                        if strength >= 0.4:
                            stable_mamba_candidate_circuits.append({
                                'indices': top_dims,
                                'type': f'stable_mamba_sae_{task_name}',
                                'source': 'stable_mamba_sae',
                                'strength': strength
                            })
                            logger.info(f"      ✅ Added StableMamba SAE circuit: {len(top_dims)} dims, strength={strength:.3f} (Task: {task_name})")
                        else:
                            logger.info(f"      ⚠️  StableMamba SAE circuit strength {strength:.3f} for task {task_name} below threshold 0.4. Skipping.")
            else:
                # Flat structure (current issue) - treat as single task
                if 'top_correlated_dims' in stable_mamba_sae_results:
                    top_dims = stable_mamba_sae_results['top_correlated_dims'][:10]
                    top_corrs = stable_mamba_sae_results.get('correlations', [0]*10)[:10]
                    ...
                    if strength >= 0.4:
                        stable_mamba_candidate_circuits.append({
                            'indices': top_dims,
                            'type': 'stable_mamba_sae_magnitude',  # Default to magnitude
                            'source': 'stable_mamba_sae',
                            'strength': strength
                        })
                        logger.info(f"      ✅ Added StableMamba SAE circuit: {len(top_dims)} dims, strength={strength:.3f}")
                    else:
                        logger.info(f"      ⚠️  StableMamba SAE circuit strength {strength:.3f} below threshold 0.4")
                else:
                    logger.warning(f"   ⚠️ StableMamba SAE results missing 'top_correlated_dims' field")
        
        # Check for StableMamba probe results
        stable_mamba_probe_file = self.experiment_logger.experiment_dir / f"stable_mamba_probe_results_layer_{layer_idx}_magnitude.json"
        if stable_mamba_probe_file.exists():
            logger.info(f"   Found StableMamba probe file: {stable_mamba_probe_file}")
            with open(stable_mamba_probe_file, 'r') as f:
                stable_mamba_probe_results = json.load(f)
            
            if 'top_correlated_dims' in stable_mamba_probe_results:
                top_dims = stable_mamba_probe_results['top_correlated_dims'][:10]
                top_corrs = stable_mamba_probe_results.get('top_correlations', [0]*10)[:10]
                strength = float(np.mean([abs(c) for c in top_corrs]))
                
                stable_mamba_candidate_circuits.append({
                    'indices': top_dims,
                    'type': 'stable_mamba_probe_magnitude',
                    'source': 'stable_mamba_probe',
                    'strength': strength
                })
                logger.info(f"      ✅ Added StableMamba probe circuit: {len(top_dims)} dims, strength={strength:.3f}")
        
        # Create result structure
        result = {
            'circuits': stable_mamba_candidate_circuits,
            'count': len(stable_mamba_candidate_circuits),
            'architecture': 'stable_mamba',
            'layer_idx': layer_idx
        }
        
        logger.info(f"✅ StableMamba circuit selection complete: {len(stable_mamba_candidate_circuits)} circuits")
        return result
    
    def test_circuit_causality_stable_mamba(self, layer_idx: int = 0):
        """Test circuit causality for StableMamba ONLY using activation patching"""
        logger.info(f"Step 6b: Testing StableMamba circuit causality with activation patching for layer {layer_idx}...")
        
        # Load StableMamba circuit candidates DIRECTLY from file
        stable_mamba_circuits_file = self.experiment_logger.experiment_dir / f"stable_mamba_candidate_circuits_layer_{layer_idx}.json"
        if not stable_mamba_circuits_file.exists():
            logger.warning(f"No StableMamba circuit candidates found for layer {layer_idx}")
            return {'architecture': 'stable_mamba', 'layer_idx': layer_idx, 'circuits_tested': 0}
        
        with open(stable_mamba_circuits_file, 'r') as f:
            stable_mamba_circuits_data = json.load(f)
        
        # Extract ONLY StableMamba circuits
        if 'circuits' not in stable_mamba_circuits_data:
            logger.warning(f"StableMamba circuits file missing 'circuits' key")
            return {'architecture': 'stable_mamba', 'layer_idx': layer_idx, 'circuits_tested': 0}
        
        stable_mamba_circuit_candidates = stable_mamba_circuits_data['circuits']
        
        if not stable_mamba_circuit_candidates:
            logger.warning(f"No StableMamba circuit candidates available")
            return {'architecture': 'stable_mamba', 'layer_idx': layer_idx, 'circuits_tested': 0}
        
        logger.info(f"Found {len(stable_mamba_circuit_candidates)} StableMamba circuits to test")
        
        # ✅ CRITICAL: Test ONLY StableMamba circuits, not original Mamba
        # Prepare test inputs
        test_texts = [
            "The quick brown fox jumps over the lazy dog.",
            "Artificial intelligence is transforming industries worldwide.",
        ...
        # Extract ONLY StableMamba circuit indices
        stable_mamba_circuit_indices = [self._extract_indices_from_circuit(c) for c in stable_mamba_circuit_candidates]
        
        # Run activation patching on ONLY StableMamba circuits
        try:
            patching_results = run_activation_patching_analysis(
                model=self.model,
                inputs=test_inputs[0]['input_ids'],
                candidate_circuits=stable_mamba_circuit_indices,  # ONLY StableMamba
                layer_idx=layer_idx,
                reference_inputs=test_inputs[1]['input_ids']
            )
            
            # Merge results with StableMamba metadata
            enhanced_results = {
                'architecture': 'stable_mamba',
                'layer_idx': layer_idx,
                'circuits_tested': len(stable_mamba_circuit_candidates),
                'circuits': {}
            }
            
            for i, circuit_meta in enumerate(stable_mamba_circuit_candidates):
                ...
                if 'circuit_results' in patching_results and circuit_key in patching_results['circuit_results']:
                    ...
                    logger.warning(f"StableMamba circuit {i} not in patching results")
            
            # Save results
            self.experiment_logger.save_results(
                enhanced_results, 
                f"stable_mamba_patching_results_layer_{layer_idx}.json"
            )
            
            logger.info(f"✅ StableMamba circuit causality testing complete! Tested {len(stable_mamba_circuit_candidates)} circuits")
            return enhanced_results
            
        except Exception as e:
            logger.error(f"StableMamba circuit causality testing failed: {e}")
            return {'architecture': 'stable_mamba', 'layer_idx': layer_idx, 'error': str(e)}
    
    def analyze_memory_horizons_stable_mamba(self, layer_idx: int = 0):
        """Analyze memory horizons for StableMamba using StableMamba-specific logic"""
        logger.info(f"Step 7b: Analyzing StableMamba memory horizons for layer {layer_idx}...")
        
        if not hasattr(self, 'stable_mamba_activation_data') or layer_idx not in self.stable_mamba_activation_data:
            logger.warning(f"No StableMamba activation data found for layer {layer_idx}")
            return {'architecture': 'stable_mamba', 'layer_idx': layer_idx, 'error': 'No activation data'}
        
        try:
            # Use the same logic as regular Mamba but for StableMamba activations
            stable_mamba_memory_horizons = self.analyze_memory_horizons(layer_idx)
            
            # Add StableMamba-specific metadata
            stable_mamba_memory_horizons['architecture'] = 'stable_mamba'
            stable_mamba_memory_horizons['layer_idx'] = layer_idx
            
            return stable_mamba_memory_horizons
        except Exception as e:
            logger.error(f"StableMamba memory horizons analysis failed: {e}")
            return {'architecture': 'stable_mamba', 'layer_idx': layer_idx, 'error': str(e)}
    
    def analyze_temporal_causality_stable_mamba(self, layer_idx: int = 0) -> Dict[str, Any]:
        """
        Step 8b: Temporal causality analysis for StableMamba using StableMamba-specific logic.
        ...
        Returns:
            Temporal analysis results for StableMamba
        """
        logger.info(f"Step 8b: Analyzing StableMamba temporal causality for layer {layer_idx}...")
        ...
        try:
            ...
            # Get StableMamba activations for this layer
            stable_mamba_activations = self.stable_mamba_activation_data[layer_idx]
            
            # Compute temporal causality using StableMamba-specific approach
            temporal_results = self._compute_stable_mamba_temporal_causality(
                test_input, stable_mamba_activations, layer_idx
            )
            
            # Add StableMamba-specific metadata
            temporal_results['architecture'] = 'stable_mamba'
            temporal_results['layer_idx'] = layer_idx
            
            # Store results
            if not hasattr(self, 'stable_mamba_temporal_results'):
                self.stable_mamba_temporal_results = {}
            self.stable_mamba_temporal_results[layer_idx] = temporal_results
            
            logger.info("✅ StableMamba temporal causality analysis complete!")
            return temporal_results
            
        except Exception as e:
            logger.error(f"StableMamba temporal causality analysis failed: {e}")
            return {'architecture': 'stable_mamba', 'layer_idx': layer_idx, 'error': str(e)}
    
    def _compute_stable_mamba_temporal_causality(self, inputs: torch.Tensor, activations: torch.Tensor, layer_idx: int) -> Dict[str, Any]:
        """
        Compute temporal causality for StableMamba using activation-based analysis.
        
        Args:
            inputs: Input tokens
            activations: StableMamba activations for the layer
            layer_idx: Layer index
            
        Returns:
            Temporal causality results
        """
        logger.info(f"Computing StableMamba temporal causality for layer {layer_idx}")
    ...
    def run_causal_equivalence_analysis_stable_mamba(self, layer_idx: int = 0) -> Dict[str, Any]:
        """
        Step 9b: Causal equivalence analysis for StableMamba by transferring activations between architectures.
        
        Args:
            layer_idx: Layer to analyze
            
        Returns:
            Causal equivalence results for StableMamba
        """
        logger.info(f"Step 9b: Running StableMamba causal equivalence analysis for layer {layer_idx}...")
        ...
        try:
            # Use the same logic as regular Mamba but for StableMamba
            causal_eq_results = self.run_causal_equivalence_analysis(layer_idx)
            
            # Add StableMamba-specific metadata
            causal_eq_results['architecture'] = 'stable_mamba'
            causal_eq_results['layer_idx'] = layer_idx
            
            # Store results
            if not hasattr(self, 'stable_mamba_causal_equivalence_results'):
                self.stable_mamba_causal_equivalence_results = {}
            self.stable_mamba_causal_equivalence_results[layer_idx] = causal_eq_results
    ...
    def run_dynamic_universality_analysis_stable_mamba(self, layer_idx: int = 0) -> Dict[str, Any]:
        """
        Step 10b: Dynamics-aware universality analysis for StableMamba with temporal signature comparison.
        
        Args:
            layer_idx: Layer to analyze
            
        Returns:
            Dynamic universality results for StableMamba
        """
        logger.info(f"Step 10b: Running StableMamba dynamic universality analysis for layer {layer_idx}...")
        ...
        try:
            ...
            # Add StableMamba-specific metadata
            dyn_uni_results['architecture'] = 'stable_mamba'
            dyn_uni_results['layer_idx'] = layer_idx
            
            # Store results
            if not hasattr(self, 'stable_mamba_dynamic_universality_results'):
                self.stable_mamba_dynamic_universality_results = {}
            self.stable_mamba_dynamic_universality_results[layer_idx] = dyn_uni_results
            
            logger.info("✅ StableMamba dynamic universality analysis complete!")
            return dyn_uni_results
            
        except Exception as e:
            logger.error(f"StableMamba dynamic universality analysis failed: {e}")
            return {'architecture': 'stable_mamba', 'layer_idx': layer_idx, 'error': str(e)}
    
    def run_mechanistic_diagnostics_stable_mamba(self, layer_idx: int = 0) -> Dict[str, Any]:
        """
        Step 11b: Enhanced mechanistic diagnostics for StableMamba architecture.
        
        Args:
            layer_idx: Layer to analyze
            
        Returns:
            Mechanistic diagnostics results for StableMamba
        """
        logger.info(f"Step 11b: Running StableMamba mechanistic diagnostics for layer {layer_idx}...")
        ...
        try:
            ...
            # Add StableMamba-specific metadata
            mechanistic_results['architecture'] = 'stable_mamba'
            mechanistic_results['layer_idx'] = layer_idx
            
            # Store results
            if not hasattr(self, 'stable_mamba_mechanistic_results'):
                self.stable_mamba_mechanistic_results = {}
            self.stable_mamba_mechanistic_results[layer_idx] = mechanistic_results
            
            logger.info("✅ StableMamba mechanistic diagnostics complete!")
            return mechanistic_results
            
        except Exception as e:
            logger.error(f"StableMamba mechanistic diagnostics failed: {e}")
            return {'architecture': 'stable_mamba', 'layer_idx': layer_idx, 'error': str(e)}
    
    def analyze_feature_superposition_stable_mamba(self, layer_idx: int = 0) -> Dict[str, Any]:
        """
        Step 12b: Feature superposition analysis for StableMamba.
        
        Args:
            layer_idx: Layer to analyze
            
        Returns:
            Feature superposition results for StableMamba
        """
        logger.info(f"Step 12b: Analyzing StableMamba feature superposition for layer {layer_idx}...")
        ...
        try:
            ...
            # Add StableMamba-specific metadata
            superposition_results['architecture'] = 'stable_mamba'
            superposition_results['layer_idx'] = layer_idx
            
            # Store results
            if not hasattr(self, 'stable_mamba_superposition_results'):
                self.stable_mamba_superposition_results = {}
            self.stable_mamba_superposition_results[layer_idx] = superposition_results
            
            logger.info("✅ StableMamba feature superposition analysis complete!")
            return superposition_results
            
        except Exception as e:
            logger.error(f"StableMamba feature superposition analysis failed: {e}")
            return {'architecture': 'stable_mamba', 'layer_idx': layer_idx, 'error': str(e)}
    
    def run_stable_mamba_dictionary_learning(self, layer_idx: int = 0, n_components: int = 512) -> Dict[str, Any]:
        """
        Run dictionary learning specifically on StableMamba activations
        """
        from sklearn.decomposition import DictionaryLearning
        import numpy as np
        
        logger.info(f"Step 13b: Running StableMamba dictionary learning for layer {layer_idx}...")
        
        # Check if StableMamba activations exist
        if not hasattr(self, 'stable_mamba_activation_data') or layer_idx not in self.stable_mamba_activation_data:
            logger.warning(f"No StableMamba activation data found for layer {layer_idx}")
            logger.info("Collecting StableMamba activations now...")
            
            # Collect StableMamba activations
            try:
                # Get sample texts
                texts = [
                    "The quick brown fox jumps over the lazy dog.",
                ...
                for text in texts[:100]:
                    ...
                    with torch.no_grad():
                        ...
                        # Run through StableMamba layer
                        if hasattr(self.model.backbone.layers[layer_idx], 'stable_mamba'):
                            stable_mamba_output = self.model.backbone.layers[layer_idx].stable_mamba(
                                hidden_states[0], layer_idx
                            )
                            all_stable_mamba_activations.append(stable_mamba_output.cpu().numpy())
                
                # Concatenate all activations
                if all_stable_mamba_activations:
                    stable_mamba_activations = np.concatenate([
                        a.reshape(-1, a.shape[-1]) for a in all_stable_mamba_activations
                    ], axis=0)
                    
                    # Store for future use
                    if not hasattr(self, 'stable_mamba_activation_data'):
                        self.stable_mamba_activation_data = {}
                    self.stable_mamba_activation_data[layer_idx] = torch.from_numpy(stable_mamba_activations)
                    
                    logger.info(f"✅ Collected StableMamba activations: {stable_mamba_activations.shape}")
                else:
                    logger.error("Failed to collect StableMamba activations")
                    return {'error': 'no_activations', 'sequences_analyzed': 0}
                    
            except Exception as e:
                logger.error(f"Failed to collect StableMamba activations: {e}")
                return {'error': str(e), 'sequences_analyzed': 0}
        
        # Get StableMamba activations
        stable_mamba_activations = self.stable_mamba_activation_data[layer_idx].cpu().numpy()
        
        logger.info(f"Running dictionary learning on StableMamba activations: {stable_mamba_activations.shape}")
        ...
        # Learn dictionary
        sparse_codes = dict_learner.fit_transform(stable_mamba_activations)
        ...
        logger.info("Generating StableMamba activation visualizations...")
        viz_results = self._generate_activation_visualizations(stable_mamba_activations, layer_idx)
        
        # Prepare results
        comprehensive_results = {
            'model_name': 'StableMamba',
            'layer_analyzed': layer_idx,
            'architecture': 'stable_mamba',
            'positional_activations': [stable_mamba_activations.tolist()],
            'dictionary_learning': {
                'n_components': n_components,
        ...
        # Generate summary
        logger.info("Generating StableMamba dictionary learning summary...")
        self._summarize_stable_mamba_dictionary_learning(comprehensive_results)
        
        # Save results
        self.experiment_logger.save_results(
            comprehensive_results, 
            f"stable_mamba_sequence_dynamics_layer_{layer_idx}.json"
        )
        
        return comprehensive_results['dictionary_learning']
    
    def _summarize_stable_mamba_dictionary_learning(self, results: Dict[str, Any]):
        """Create summary for StableMamba dictionary learning"""
        import numpy as np
        
        logger.info("=" * 60)
        logger.info("📊 STEP 13b: STABLE_MAMBA DICTIONARY LEARNING RESULTS SUMMARY")
        logger.info("=" * 60)
        ...
        logger.info(f"\n📈 STABLE_MAMBA ACTIVATION STATISTICS:")
        ...
        logger.info(f"\n🔝 TOP 5 MOST VARIABLE STABLE_MAMBA NEURONS:")
        for i, neuron in enumerate(top_5_neurons[::-1]):
            logger.info(f"   {i+1}. Neuron {neuron}: var={neuron_variance[neuron]:.4f}")
        
        logger.info("=" * 60)
    
    def _print_stable_mamba_dictionary_learning_results(self, results: Dict[str, Any], layer_idx: int):
        """Print detailed StableMamba dictionary learning results similar to original Mamba"""
        import numpy as np
        ...
        logger.info(f"🧠 Model: {results.get('model_name', 'StableMamba')}")
        logger.info(f"🔬 Layer: {layer_idx}")
        logger.info(f"🏗️ Architecture: {results.get('architecture', 'stable_mamba')}")
        logger.info(f"📝 Sequences Analyzed: {len(results.get('positional_activations', []))}")
    ...
    def compare_across_model_scales_stable_mamba(self) -> Dict[str, Any]:
        """
        Step 14b: Scaling analysis for StableMamba across different model sizes.
        
        Returns:
            Scaling analysis results for StableMamba
        """
        logger.info("Step 14b: Running StableMamba scaling analysis...")
        
        try:
            # Define model sizes for StableMamba scaling analysis
            model_sizes = [
            ...
            # Add StableMamba-specific metadata
            scaling_results['architecture'] = 'stable_mamba'
            
            # Store results
            if not hasattr(self, 'stable_mamba_scaling_results'):
                self.stable_mamba_scaling_results = scaling_results
            
            logger.info("✅ StableMamba scaling analysis complete!")
            return scaling_results
            
        except Exception as e:
            logger.error(f"StableMamba scaling analysis failed: {e}")
            return {'architecture': 'stable_mamba', 'error': str(e)}
    
    def run_grokking_analysis_stable_mamba(self, layer_idx: int = 0) -> Dict[str, Any]:
        """
        Step 15b: Grokking analysis for StableMamba.
        
        Args:
            layer_idx: Layer to analyze
            
        Returns:
            Grokking analysis results for StableMamba
        """
        logger.info(f"Step 15b: Running StableMamba grokking analysis for layer {layer_idx}...")
        ...
        try:
            ...
            # Add StableMamba-specific metadata
            grokking_results['architecture'] = 'stable_mamba'
            grokking_results['layer_idx'] = layer_idx
            
            # Store results
            if not hasattr(self, 'stable_mamba_grokking_results'):
                self.stable_mamba_grokking_results = {}
            self.stable_mamba_grokking_results[layer_idx] = grokking_results
            
            logger.info("✅ StableMamba grokking analysis complete!")
            return grokking_results
            
        except Exception as e:
            logger.error(f"StableMamba grokking analysis failed: {e}")
            return {'architecture': 'stable_mamba', 'layer_idx': layer_idx, 'error': str(e)}
    
    def visualize_sparse_probing_stable_mamba(self, layer_idx: int = 0) -> Dict[str, Any]:
        """
        Step 16b: Sparse probing visualization for StableMamba.
        
        Args:
            layer_idx: Layer to analyze
            
        Returns:
            Sparse probing visualization results for StableMamba
        """
        logger.info(f"Step 16b: Running StableMamba sparse probing visualization for layer {layer_idx}...")
        ...
        try:
            ...
            # Add StableMamba-specific metadata
            probe_viz_results['architecture'] = 'stable_mamba'
            probe_viz_results['layer_idx'] = layer_idx
            
            # Store results
            if not hasattr(self, 'stable_mamba_probe_viz_results'):
                self.stable_mamba_probe_viz_results = {}
            self.stable_mamba_probe_viz_results[layer_idx] = probe_viz_results
            
            logger.info("✅ StableMamba sparse probing visualization complete!")
            return probe_viz_results
            
        except Exception as e:
            logger.error(f"StableMamba sparse probing visualization failed: {e}")
            return {'architecture': 'stable_mamba', 'layer_idx': layer_idx, 'error': str(e)}
    
    def run_spd_analysis_stable_mamba(self, layer_idx: int = 0) -> Dict[str, Any]:
        """
        Step 17b: Stochastic Parameter Decomposition (SPD) analysis for StableMamba.
        
        Args:
            layer_idx: Layer to analyze
            
        Returns:
            SPD analysis results for StableMamba
        """
        logger.info(f"Step 17b: Running StableMamba SPD analysis for layer {layer_idx}...")
        ...
        try:
            # Use the proper SPDAnalyzer for StableMamba, same as regular Mamba
            # Ensure analyzer is set up before creating SPD analyzer
            if self.model is None:
                logger.info("Setting up analyzer before StableMamba SPD analysis...")
                self.setup()
            ...
            # Add StableMamba-specific metadata
            spd_results['architecture'] = 'stable_mamba'
            spd_results['method'] = 'stochastic_parameter_decomposition'
            
            logger.info(f"StableMamba SPD analysis complete for layer {layer_idx}")
            ...
            # Store results
            if not hasattr(self, 'stable_mamba_spd_results'):
                self.stable_mamba_spd_results = {}
            self.stable_mamba_spd_results[layer_idx] = spd_results
            
            logger.info("✅ StableMamba SPD analysis complete!")
            return spd_results
            
        except Exception as e:
            logger.error(f"StableMamba SPD analysis failed: {e}")
            return {'architecture': 'stable_mamba', 'layer_idx': layer_idx, 'error': str(e)}
    
    def run_apd_analysis_stable_mamba(self, layer_idx: int = 0) -> Dict[str, Any]:
        """
        Step 18b: Attribution-based Parameter Decomposition (APD) analysis for StableMamba.
        
        Args:
            layer_idx: Layer to analyze
            
        Returns:
            APD analysis results for StableMamba
        """
        logger.info(f"Step 18b: Running StableMamba APD analysis for layer {layer_idx}...")
        ...
        try:
            ...
            # Store results
            if not hasattr(self, 'stable_mamba_apd_results'):
                self.stable_mamba_apd_results = {}
            self.stable_mamba_apd_results[layer_idx] = apd_results
            
            logger.info("✅ StableMamba APD analysis complete!")
            return apd_results
            
        except Exception as e:
            logger.error(f"StableMamba APD analysis failed: {e}")
            return {'architecture': 'stable_mamba', 'layer_idx': layer_idx, 'error': str(e)}
    
    def run_post_spd_analysis_stable_mamba(self, layer_idx: int = 0) -> Dict[str, Any]:
        """
        Step 19b: Comprehensive post-SPD cluster analysis for StableMamba.
        
        Args:
            layer_idx: Layer to analyze
            
        Returns:
            Comprehensive post-SPD analysis results for StableMamba
        """
        logger.info(f"Step 19b: Running comprehensive StableMamba post-SPD cluster analysis for layer {layer_idx}...")
        ...
        try:
            # Get StableMamba SPD results
            if not hasattr(self, 'stable_mamba_spd_results') or layer_idx not in self.stable_mamba_spd_results:
                logger.warning(f"No StableMamba SPD results found for layer {layer_idx}")
                return {'architecture': 'stable_mamba', 'layer_idx': layer_idx, 'error': 'No SPD results'}
            
            spd_results = self.stable_mamba_spd_results[layer_idx]
            
            # 19a: StableMamba layer specialization analysis
            logger.info("Step 19a: Analyzing StableMamba layer specialization...")
            layer_specialization = self._analyze_stable_mamba_layer_specialization(layer_idx)
            
            # 19b: StableMamba cluster ablation analysis
            logger.info("Step 19b: Running StableMamba cluster ablation analysis...")
            cluster_ablations = {}
            if 'clusters' in spd_results and 'clusters' in spd_results['clusters']:
                ...
                for cluster_id in range(min(8, n_clusters)):
                    try:
                        ablation_result = self._ablate_stable_mamba_cluster(cluster_id, spd_results, layer_idx)
                        cluster_ablations[str(cluster_id)] = ablation_result
                    except Exception as e:
                        logger.warning(f"Failed to ablate StableMamba cluster {cluster_id}: {e}")
                        cluster_ablations[str(cluster_id)] = {'error': str(e)}
            
            # 19c: StableMamba cluster interactions analysis
            logger.info("Step 19c: Measuring StableMamba cluster interactions...")
            try:
                cluster_interactions = self._measure_stable_mamba_cluster_interactions(spd_results, layer_idx)
            except Exception as e:
                logger.warning(f"StableMamba cluster interactions analysis failed: {e}")
                cluster_interactions = None
            
            # 19d: StableMamba information bottleneck analysis
            logger.info("Step 19d: Analyzing StableMamba information bottleneck...")
            try:
                information_bottleneck = self._analyze_stable_mamba_information_bottleneck(layer_idx)
            except Exception as e:
                logger.warning(f"StableMamba information bottleneck analysis failed: {e}")
                information_bottleneck = None
            
            # 19e: StableMamba critical parameter ablation
            logger.info("Step 19e: Ablating StableMamba critical parameter...")
            try:
                critical_param_ablation = self._ablate_stable_mamba_critical_parameter(spd_results, layer_idx)
            except Exception as e:
                logger.warning(f"StableMamba critical parameter ablation failed: {e}")
                critical_param_ablation = None
            
            # 19f: StableMamba gradient flow analysis
            logger.info("Step 19f: Analyzing StableMamba gradient flow...")
            try:
                gradient_flow_analysis = self._analyze_stable_mamba_gradient_flow(layer_idx)
            except Exception as e:
                logger.warning(f"StableMamba gradient flow analysis failed: {e}")
                gradient_flow_analysis = None
            
            # 19g: StableMamba functional analysis
            logger.info("Step 19g: Analyzing StableMamba functional behavior...")
            try:
                functional_analysis = self._analyze_stable_mamba_functional_behavior(spd_results, layer_idx)
            except Exception as e:
                logger.warning(f"StableMamba functional analysis failed: {e}")
            ...
            # Store results
            if not hasattr(self, 'stable_mamba_post_spd_results'):
                self.stable_mamba_post_spd_results = {}
            self.stable_mamba_post_spd_results[layer_idx] = post_spd_results
            
            logger.info("✅ Comprehensive StableMamba post-SPD cluster analysis complete!")
            return post_spd_results
            
        except Exception as e:
            logger.error(f"StableMamba post-SPD cluster analysis failed: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return {'architecture': 'stable_mamba', 'layer_idx': layer_idx, 'error': str(e)}
    
    def _analyze_stable_mamba_layer_specialization(self, layer_idx: int) -> Dict[str, Any]:
        """Analyze StableMamba layer specialization patterns"""
        try:
            if not hasattr(self, 'stable_mamba_activation_data') or layer_idx not in self.stable_mamba_activation_data:
                return {'error': 'No activation data'}
            
            activations = self.stable_mamba_activation_data[layer_idx]
            specialization = {}
            ...
            logger.warning(f"StableMamba layer specialization analysis failed: {e}")
            return {'error': str(e)}
    
    def _ablate_stable_mamba_cluster(self, cluster_id: int, spd_results: Dict, layer_idx: int) -> Dict[str, Any]:
        """Ablate a specific StableMamba parameter cluster"""
        try:
            ...
            logger.warning(f"StableMamba cluster ablation failed: {e}")
            return {'error': str(e)}
    
    def _measure_stable_mamba_cluster_interactions(self, spd_results: Dict, layer_idx: int) -> List[List[float]]:
        """Measure interactions between StableMamba parameter clusters"""
        try:
            if 'clusters' not in spd_results or 'clusters' not in spd_results['clusters']:
                return None
    ...
    def _analyze_stable_mamba_information_bottleneck(self, layer_idx: int) -> Dict[str, Any]:
        """Analyze information bottleneck in StableMamba"""
        try:
            if not hasattr(self, 'stable_mamba_activation_data') or layer_idx not in self.stable_mamba_activation_data:
                return {'error': 'No activation data'}
    ...
    def _ablate_stable_mamba_critical_parameter(self, spd_results: Dict, layer_idx: int) -> Dict[str, Any]:
        """Ablate the most critical StableMamba parameter"""
        try:
    ...
    def _analyze_stable_mamba_gradient_flow(self, layer_idx: int) -> Dict[str, Any]:
        """Analyze gradient flow in StableMamba"""
        try:
            # Get StableMamba layer
            wrapped_layer = self.model.backbone.layers[layer_idx]
            if not hasattr(wrapped_layer, 'stable_mamba'):
                return {'error': 'No StableMamba layer found'}
            
            stable_mamba = wrapped_layer.stable_mamba
            ...
            for name, param in stable_mamba.named_parameters():
    ...
    def _analyze_stable_mamba_functional_behavior(self, spd_results: Dict, layer_idx: int) -> Dict[str, Any]:
        """Analyze functional behavior of key StableMamba parameters"""
        try:
            # Focus on key StableMamba parameters
            key_params = [
                f'backbone.layers.{layer_idx}.stable_mamba.grad_scale',
                f'backbone.layers.{layer_idx}.stable_mamba.residual_scale',
                f'backbone.layers.{layer_idx}.stable_mamba.gate_weights',
    ...
    def select_candidate_circuits(self, layer_idx: int = 0) -> List[Dict[str, Any]]:
        ...
        # NEW: Select candidate circuits for StableMamba if available
        if hasattr(self, 'stable_mamba_activation_data') and layer_idx in self.stable_mamba_activation_data:
            logger.info(f"Step 5b: Selecting candidate circuits for StableMamba layer {layer_idx}...")
            stable_mamba_candidate_circuits = []
            ...
            if stable_mamba_sae_file.exists():
                ...
                if isinstance(stable_mamba_sae_results, dict) and any(isinstance(v, dict) for v in stable_mamba_sae_results.values()):
                    ...
                    if 'top_correlated_dims' in stable_mamba_sae_results:
                        top_dims = stable_mamba_sae_results['top_correlated_dims'][:10]
                        top_corrs = stable_mamba_sae_results.get('top_correlations', [0]*10)[:10]
                        strength = float(np.mean([abs(c) for c in top_corrs]))
            ...
            # Save StableMamba circuit candidates
            self.experiment_logger.save_results(
                {'circuits': stable_mamba_candidate_circuits, 'count': len(stable_mamba_candidate_circuits)},
                f"stable_mamba_candidate_circuits_layer_{layer_idx}.json"
            )
            logger.info(f"✅ StableMamba circuit selection complete: {len(stable_mamba_candidate_circuits)} circuits")
        else:
            logger.warning(f"No StableMamba activation data found for layer {layer_idx}")
    ...
    def _load_model_and_tokenizer(self):
        ...
        # Check if we should load a pre-steered model
        steered_model_path = getattr(self.config, 'steered_model_path', None)
        if steered_model_path:
            logger.info(f"📦 Loading pre-steered model from: {steered_model_path}")
            try:
                model, tokenizer, steering_config = load_steered_model(
                    steered_model_path,
                    device=self.config.device
                )
                # If it's a SteeredMambaModel, extract the steering
                if isinstance(model, SteeredMambaModel):
                    self.steering = model.steering
                    self.is_steered = True
                    logger.info("✅ Loaded steered model with steering hooks active")
                    logger.info(f"   Active hooks: {len(self.steering.hooks)}")
                    logger.info(f"   Steering layers: {steering_config.get('layer_indices', [])}")
                    logger.info(f"   Steering strength: {steering_config.get('strength', 5.0)}x")
                else:
                    self.steering = None
                    self.is_steered = False
                return model, tokenizer
            except Exception as e:
                logger.warning(f"⚠️ Failed to load steered model: {e}")
                import traceback
        ...
        # Initialize steered mamba for interpretability analysis
        # This is optional - can be enabled/disabled via setup()
        logger.info("🔧 Initializing steered mamba...")
        try:
            self.steering = SimpleSteering(model)
            logger.info("✅ Initialized steered mamba with Cluster 9 neurons")
            logger.info(f"   Model has {len(self.steering.layers)} layers available for steering")
            logger.info(f"   Cluster 9 neurons: {len(self.steering.cluster9_neurons)} neurons")
        except Exception as e:
            logger.warning(f"⚠️ Failed to initialize steered mamba: {e}")
            import traceback
            logger.warning(traceback.format_exc())
            logger.info("   Continuing without steering (regular Mamba mode)")
            self.steering = None
            self.is_steered = False
        
        # Note: Stable Mamba layers are attached in setup() if use_stable_mamba=True
    ...
    def _force_gradient_enable(self):
        ...
        # ✅ ADD: Verify StableMamba layers are properly registered
        stable_mamba_params = sum(1 for name, _ in self.model.named_parameters() if 'stable_mamba' in name)
        logger.info(f"StableMamba parameters found: {stable_mamba_params}")
        
        if stable_mamba_params == 0:
            logger.warning("⚠️ No StableMamba parameters found! Layers may not be properly registered.")
    ...
    def _gradient_pre_warming(self, inputs: Dict[str, torch.Tensor], num_passes: int = 3):
        ...
        if best_coverage < 0.5:
            ...
            logger.warning("   This suggests gradient flow issues in StableMamba layers!")
    ...
    def test_stable_mamba_gradient_flow(self, layer_idx=0):
        """
        Test gradient flow after applying fixes
        """
        logger.info("Testing StableMamba gradient flow...")
        ...
        # Check StableMamba gradients
        stable_mamba_grads = {}
        for name, param in self.model.named_parameters():
            if 'stable_mamba' in name and param.grad is not None:
                stable_mamba_grads[name] = param.grad.norm().item()
        
        if not stable_mamba_grads:
            logger.error("❌ NO STABLE_MAMBA GRADIENTS!")
            return False
        
        logger.info(f"✅ StableMamba gradients found: {len(stable_mamba_grads)}")
        logger.info(f"   Mean gradient norm: {np.mean(list(stable_mamba_grads.values())):.6f}")
        logger.info(f"   Max gradient norm: {np.max(list(stable_mamba_grads.values())):.6f}")
        
        return np.mean(list(stable_mamba_grads.values())) > 1e-6
    ...
    def analyze_stable_mamba_parameters(self, layer_idx: int = 0) -> Dict[str, Any]:
        """
        Analyze StableMamba layer parameters (gate weights, SSM decays, etc.)
        """
        logger.info(f"Analyzing StableMamba parameters for layer {layer_idx}...")
        
        try:
            layer = self.model.backbone.layers[layer_idx]
            if not hasattr(layer, 'stable_mamba'):
                logger.warning(f"Layer {layer_idx} has no stable_mamba attribute")
                return {'error': 'no_stable_mamba_layer'}
            
            stable_mamba_layer = layer.stable_mamba
            
            # Analyze gate weights
            gate_weights = stable_mamba_layer.gate_weights.detach()
            gate_analysis = {
            ...
            ssm_decays = {
                'fast': stable_mamba_layer.ssm_fast.decay,
                'medium': stable_mamba_layer.ssm_medium.decay,
                'slow': stable_mamba_layer.ssm_slow.decay
            }
            
            # Analyze compression predictor
            comp_weight_norm = torch.norm(stable_mamba_layer.compression_predictor.weight).item()
            comp_bias_norm = torch.norm(stable_mamba_layer.compression_predictor.bias).item()
            
            # Analyze sparse attention sparsity
            sparse_sparsity = stable_mamba_layer.sparse_attn.sparsity
            ...
            results = {
                ...
                'layer_type': 'stable_mamba',
                'd_model': stable_mamba_layer.d_model
            }
            
            logger.info(f"   ✅ StableMamba parameter analysis complete")
            return results
            
        except Exception as e:
            logger.error(f"   ❌ StableMamba analysis failed: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return {'error': str(e)}
    ...
    def run_complete_analysis_with_visualizations(self, skip_steps=None):
        ...
        # NEW: StableMamba Analysis for All Layers
        logger.info("📊 Running StableMamba analysis for all layers...")
        try:
            # Get total number of layers
            total_layers = len(self.model.backbone.layers)
            logger.info(f"Analyzing StableMamba for {total_layers} layers...")
            
            # Analyze StableMamba for all layers
            stable_mamba_all_layers_results = {}
            stable_mamba_all_layers_dynamics = {}
            
            for layer_idx in range(total_layers):
                logger.info(f"Analyzing StableMamba layer {layer_idx}/{total_layers-1}...")
                
                # StableMamba SSM parameters for this layer
                stable_mamba_ssm_results = self.analyze_stable_mamba_parameters(layer_idx=layer_idx)
                stable_mamba_all_layers_results[layer_idx] = stable_mamba_ssm_results
                self.experiment_logger.save_results(stable_mamba_ssm_results, f"stable_mamba_ssm_parameters_layer_{layer_idx}.json")
                
                # StableMamba sequence dynamics for this layer
                stable_mamba_dynamics_results = self.analyze_stable_mamba_sequence_dynamics(texts[:10], layer_idx=layer_idx)
                stable_mamba_all_layers_dynamics[layer_idx] = stable_mamba_dynamics_results
                self.experiment_logger.save_results(stable_mamba_dynamics_results, f"stable_mamba_sequence_dynamics_layer_{layer_idx}.json")
            
            # StableMamba comprehensive report for all layers
            stable_mamba_comprehensive = {
                'ssm_parameters_all_layers': stable_mamba_all_layers_results,
                'sequence_dynamics_all_layers': stable_mamba_all_layers_dynamics,
                'baseline_stats': getattr(self, 'stable_mamba_activation_data', {}),
                'layer_type': 'stable_mamba',
                'total_layers_analyzed': total_layers,
                'analysis_timestamp': str(datetime.now())
            }
            self.experiment_logger.save_results(stable_mamba_comprehensive, "stable_mamba_comprehensive_report.json")
            
            viz_results['stable_mamba'] = stable_mamba_comprehensive
            logger.info(f"✅ StableMamba analysis complete for all {total_layers} layers!")
            
        except Exception as e:
            logger.warning(f"StableMamba analysis failed: {e}")
            viz_results['stable_mamba'] = {'error': str(e)}
    ...
    def analyze_stable_mamba_sequence_dynamics(self, texts: List[str], layer_idx: int = 0) -> Dict[str, Any]:
        """
        Analyze StableMamba sequence dynamics using stable_mamba layer outputs
        """
        logger.info(f"Analyzing StableMamba sequence dynamics for layer {layer_idx}...")
        ...
        try:
            ...
            for text_idx, text in enumerate(texts[:5]):
                try:
                    ...
                    with torch.no_grad():
                        ...
                        # Run through StableMamba layer
                        stable_mamba_output = stable_mamba_layer(hidden_states[0], layer_idx)  # [seq_len, hidden_dim]
                        
                        # Analyze gate activations
                        gate_outputs = [gate(hidden_states[0]) for gate in stable_mamba_layer.gates]
                        gate_weights = torch.softmax(stable_mamba_layer.gate_weights, dim=0)
                        ...
                        compression = torch.sigmoid(stable_mamba_layer.compression_predictor(seq_mean))  # [1, 1]
                    
                    # Compute state transitions for StableMamba output
                    seq_len = stable_mamba_output.shape[0]
                    
                    if seq_len > 1:
                        # Compute transitions between consecutive positions
                        transitions = []
                        for i in range(1, seq_len):
                            delta = torch.norm(stable_mamba_output[i] - stable_mamba_output[i-1]).item()
                            transitions.append(delta)
                        ...
                        # Store positional activations
                        results['positional_activations'].append(stable_mamba_output.cpu().numpy())
                        results['gate_activations'].append(combined_gates.cpu().numpy())
                        results['compression_levels'].append(compression.cpu().numpy())
                        
                        # Find critical dimensions (highest temporal variance)
                        temporal_variance = torch.var(stable_mamba_output, dim=0)
                        critical_dims = torch.argsort(temporal_variance, descending=True)[:10].tolist()
                        results['critical_dimensions'].extend(critical_dims)
                    else:
                        logger.warning(f"   Text {text_idx} only has {seq_len} token, skipping")
            ...
            # Add StableMamba-specific analysis
            ...
            results['layer_type'] = 'stable_mamba'
            return results
            
        except Exception as e:
            logger.error(f"   ❌ StableMamba sequence dynamics analysis failed: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return {'error': str(e)}
    ...
    def visualize_phase_transition(self):
        ...
        axes[0, 1].plot(layers, entropies, 's-', linewidth=4, markersize=8, color='black')
        ...
        axes[0, 1].set_title('Information Entropy Across Layers\n(Peak at Layer 20)', 
                             fontweight='bold', fontsize=14)
        axes[0, 1].set_xlabel('Layer Index', fontsize=40)
        axes[0, 1].set_ylabel('Entropy', fontsize=40)
        axes[0, 1].tick_params(axis='both', labelsize=40)
        axes[0, 1].grid(True, alpha=0.3)
        ...
        # Create separate entropy trajectory plot with custom styling
        fig_entropy, ax_entropy = plt.subplots(1, 1, figsize=(12, 8))
        
        # Set default font size for all text elements
        plt.rcParams.update({'font.size': 20})
        
        # Plot entropy trajectory with thicker black line (no markers on line)
        ax_entropy.plot(layers, entropies, '-', linewidth=8, color='black')
        
        # Add square markers only on layers 19, 20, 21
        for layer_idx in [19, 20, 21]:
            if layer_idx < len(entropies):
                ax_entropy.plot(layer_idx, entropies[layer_idx], 's', markersize=12, color='black', 
                               markeredgecolor='black', markeredgewidth=2)
        
        ax_entropy.axvspan(19.5, 21.5, alpha=0.2, color='red')
        ax_entropy.axvline(x=20, color='red', linestyle='--', linewidth=6)
        ax_entropy.set_title('Information Entropy Across Layers\n(Peak at Layer 20)', 
                            fontweight='bold', fontsize=20)
        ax_entropy.set_xlabel('Layer Index', fontsize=25, fontweight='bold')
        ax_entropy.set_ylabel('Entropy', fontsize=25, fontweight='bold')
        
        # Set x-axis ticks to only show 0, 10, 20
        ax_entropy.set_xticks([0, 10, 20])
        
        # Reduce y-axis ticks (use fewer major ticks)
        ax_entropy.locator_params(axis='y', nbins=5)
        
        ax_entropy.grid(True, alpha=0.3)
        ax_entropy.tick_params(axis='both', which='major', labelsize=25, width=2)
        
        # Make tick labels bold
        for label in ax_entropy.get_xticklabels():
            label.set_fontweight('bold')
        for label in ax_entropy.get_yticklabels():
            label.set_fontweight('bold')
        
        plt.tight_layout()
        entropy_save_path = self.experiment_logger.experiment_dir / 'mamba_entropy_trajectory.png'
        plt.savefig(entropy_save_path, dpi=300, bbox_inches='tight')
        plt.close()
        
        logger.info(f"Separate entropy trajectory plot saved: {entropy_save_path}")
...
def main():
    ...
    parser.add_argument('--steered_model', type=str, default=None,
                       help='Path to pre-steered model directory (if provided, will use steered model instead of regular)')
    parser.add_argument('--mamba', action='store_true',
                       help='Run regular Mamba analysis (default if no other mode specified)')
    parser.add_argument('--steeredmamba', action='store_true',
                       help='Run steered Mamba analysis (Cluster 9 neurons amplified)')
    parser.add_argument('--stablemamba', action='store_true',
                       help='Run stable Mamba analysis (enhanced architecture with multi-gate and multi-timescale)')
    ...
    # Set steered model path if provided
    if args.steered_model:
        config.steered_model_path = args.steered_model
        logger.info(f"📦 Will use pre-steered model from: {args.steered_model}")
    
    # Determine which mode(s) to run
    # Count how many modes are specified
    modes_specified = sum([args.mamba, args.steeredmamba, args.stablemamba])
    
    if modes_specified == 0:
        # DEFAULT: Run regular Mamba if no mode specified
        args.mamba = True
        logger.info("📊 DEFAULT MODE: Running regular Mamba (use --mamba, --steeredmamba, or --stablemamba to specify)")
        print("📊 DEFAULT MODE: Running regular Mamba")
    elif modes_specified > 1:
        logger.warning(f"⚠️ Multiple modes specified! Only one mode should be used at a time.")
        logger.warning(f"   Specified: mamba={args.mamba}, steeredmamba={args.steeredmamba}, stablemamba={args.stablemamba}")
        logger.warning(f"   Using first specified mode...")
    
    # Set mode flags in config
    config.use_mamba = args.mamba
    config.use_steered_mamba = args.steeredmamba
    config.use_stable_mamba = args.stablemamba
    
    # Log selected mode
    if args.mamba:
        logger.info("📊 Mode: REGULAR MAMBA")
        print("📊 Mode: REGULAR MAMBA")
    elif args.steeredmamba:
        logger.info("📊 Mode: STEERED MAMBA (Cluster 9 neurons amplified)")
        print("📊 Mode: STEERED MAMBA")
    elif args.stablemamba:
        logger.info("📊 Mode: STABLE MAMBA (Enhanced architecture)")
        print("📊 Mode: STABLE MAMBA")
    ...
    try:
        ...
        if args.complete_analysis:
            ...
            # Save complete analysis visualization results
            if viz_results:
                analyzer.experiment_logger.save_results(viz_results, "complete_analysis_visualizations.json")
            logger.info("🎉 Complete analysis with visualizations finished!")
            return
        ...
        if args.stablemamba:
            # Stable Mamba mode
            logger.info("=" * 80)
            logger.info("🔄 Running STABLE MAMBA analysis")
            logger.info("=" * 80)
            print("\n" + "=" * 80)
            print("🔄 Running STABLE MAMBA analysis")
            print("=" * 80 + "\n")
            
            analyzer.setup(enable_steering=False, use_stable_mamba=True)
        elif args.steeredmamba:
            # Steered Mamba mode
            logger.info("=" * 80)
            logger.info("🔄 Running STEERED MAMBA analysis")
            logger.info("=" * 80)
            print("\n" + "=" * 80)
            print("🎯 Running STEERED MAMBA analysis")
            print("=" * 80 + "\n")
            
            analyzer.setup(enable_steering=True, use_stable_mamba=False)
        else:
            # Regular Mamba mode (default)
            logger.info("=" * 80)
            logger.info("🔄 Running REGULAR MAMBA analysis")
            logger.info("=" * 80)
            print("\n" + "=" * 80)
            print("🔄 Running REGULAR MAMBA analysis")
            print("=" * 80 + "\n")
            
            analyzer.setup(enable_steering=False, use_stable_mamba=False)
        
        # Legacy run_both support (deprecated, use individual flags instead)
        if getattr(args, 'run_both', False):
            logger.info("=" * 80)
            logger.info("🔄 Running in 'BOTH' mode: Regular Mamba first, then Steered Mamba")
            logger.info("=" * 80)
            print("\n" + "=" * 80)
            print("🔄 PHASE 1: Running REGULAR MAMBA analysis")
            print("=" * 80 + "\n")
            
            # Phase 1: Regular Mamba (no steering)
            analyzer.setup(enable_steering=False)
            
            # Run all analysis steps for regular Mamba
            if '2' not in args.skip_steps:
                activations = analyzer.collect_activations(texts)
            
            # Continue with other steps for regular Mamba...
            # (The rest of the analysis will run for regular Mamba)
            
            logger.info("✅ Phase 1 (Regular Mamba) complete! Files saved without 'steered_' prefix.\n")
            
            # Phase 2: Steered Mamba (with steering)
            logger.info("=" * 80)
            logger.info("🔄 PHASE 2: Running STEERED MAMBA analysis")
            logger.info("=" * 80)
            print("\n" + "=" * 80)
            print("🎯 PHASE 2: Running STEERED MAMBA analysis")
            print("=" * 80 + "\n")
            
            # Re-setup with steering enabled
            analyzer.setup(enable_steering=True)
            
            # Run all analysis steps again for steered Mamba
            if '2' not in args.skip_steps:
                activations = analyzer.collect_activations(texts)
            
            # Continue with other steps for steered Mamba...
            # (The rest of the analysis will run for steered Mamba with steered_ prefix)
            
            logger.info("✅ Phase 2 (Steered Mamba) complete!")
            print("\n✅ Phase 2 (Steered Mamba) complete! Files saved with 'steered_' prefix.\n")
            
            print("=" * 80)
            print("✅ ANALYSIS COMPLETE!")
            print("   - Regular Mamba: Files without prefix (e.g., baseline_stats.json)")
            print("   - Steered Mamba: Files with 'steered_' prefix (e.g., steered_baseline_stats.json)")
            print("=" * 80)
            return
        
        # Single-mode execution (steered_only mode)
        if getattr(config, 'run_both_regular_and_steered', False):
            logger.warning("⚠️ run_both_regular_and_steered is True but we're in single-mode execution. This shouldn't happen.")
        ...
        if '2' not in args.skip_steps:
            activations = analyzer.collect_activations(texts)
            # Results are already saved in collect_activations method (baseline_stats.json)
        ...
        if '3' not in args.skip_steps:
            ...
            if sae_results and 'sae_results' not in str(sae_results):
                analyzer.experiment_logger.save_results(sae_results, f"step3_sae_results_layer_{args.layer}.json")
        ...
        if '4' not in args.skip_steps:
            ...
            # Save results if not already saved
            if probe_results:
                analyzer.experiment_logger.save_results(probe_results, f"step4_probe_results_layer_{args.layer}.json")
        ...
        if '5' not in args.skip_steps:
            ...
            # Save circuit selection results
            if circuits:
                analyzer.experiment_logger.save_results({'circuits': circuits}, f"step5_circuit_selection_layer_{args.layer}.json")
        ...
        if '6' not in args.skip_steps:
            ...
            # Save patching results if not already saved
            if patching_results:
                analyzer.experiment_logger.save_results(patching_results, f"step6_patching_results_layer_{args.layer}.json")
        ...
        if '8' not in args.skip_steps:
            ...
            # Save temporal results if not already saved
            if temporal_results:
                analyzer.experiment_logger.save_results(temporal_results, f"step8_temporal_causality_layer_{args.layer}.json")
            logger.info("✅ Temporal causality analysis complete!")
        ...
        if '9' not in args.skip_steps:
            ...
            # Save main causal equivalence results
            if causal_eq_results:
                analyzer.experiment_logger.save_results(causal_eq_results, f"step9_causal_equivalence_layer_{args.layer}.json")
            logger.info("✅ Causal equivalence analysis complete!")
            ...
            if gap_results:
                analyzer.experiment_logger.save_results(gap_results, f"step9a_similarity_equivalence_gap_layer_{args.layer}.json")
            ...
            # Save advantages results
            if advantages:
                analyzer.experiment_logger.save_results(advantages, f"step9b_mamba_unique_advantages_layer_{args.layer}.json")
            ...
            # Save insights results
            if insights:
                analyzer.experiment_logger.save_results(insights, f"step9c_hybrid_architecture_insights_layer_{args.layer}.json")
            ...
            # Save hybrid tests results
            if hybrid_tests:
                analyzer.experiment_logger.save_results(hybrid_tests, f"step9d_hybrid_architecture_principles_layer_{args.layer}.json")
            ...
            if generalization:
                analyzer.experiment_logger.save_results(generalization, f"step9e_architectural_generalization_layer_{args.layer}.json")
            ...
            if prototype:
                analyzer.experiment_logger.save_results(prototype, f"step9f_prototype_hybrid_model_layer_{args.layer}.json")
            ...
            # Save scaling laws results
            if scaling_laws:
                analyzer.experiment_logger.save_results(scaling_laws, f"step9g_hybrid_scaling_laws_layer_{args.layer}.json")
            ...
            if blueprint:
                analyzer.experiment_logger.save_results(blueprint, f"step9h_hybrid_architecture_blueprint_layer_{args.layer}.json")
        ...
        if '10' not in args.skip_steps:
            ...
            # Save dynamic universality results
            if dyn_uni_results:
                analyzer.experiment_logger.save_results(dyn_uni_results, f"step10_dynamic_universality_layer_{args.layer}.json")
            logger.info("✅ Dynamic universality analysis complete!")
        ...
        if '11' not in args.skip_steps:
            ...
            # Save mechanistic diagnostics results
            if off_by_one_results:
                analyzer.experiment_logger.save_results(off_by_one_results, f"step11_mechanistic_diagnostics_layer_{args.layer}.json")
        ...
        if '12' not in args.skip_steps:
            ...
            if superposition_results:
                analyzer.experiment_logger.save_results(superposition_results, f"step12_feature_superposition_layer_{args.layer}.json")
            ...
            if viz_results:
                analyzer.experiment_logger.save_results(viz_results, f"step12_superposition_visualization_layer_{args.layer}.json")
        ...
        if '13' not in args.skip_steps:
            ...
            # Save dictionary learning results
            if dict_results:
                analyzer.experiment_logger.save_results(dict_results, f"step13_dictionary_learning_layer_{args.layer}.json")
            logger.info("✅ Dictionary learning complete!")
            
            # Step 13b: StableMamba Dictionary Learning
            logger.info("Step 13b: Running StableMamba dictionary learning...")
            stable_mamba_dict_results = analyzer.run_stable_mamba_dictionary_learning(args.layer)
            # Save StableMamba dictionary learning results
            if stable_mamba_dict_results:
                analyzer.experiment_logger.save_results(stable_mamba_dict_results, f"step13b_stable_mamba_dictionary_learning_layer_{args.layer}.json")
            logger.info("✅ StableMamba dictionary learning complete!")
        ...
        if '14' not in args.skip_steps:
            ...
            if scaling_results:
                analyzer.experiment_logger.save_results(scaling_results, f"step14_scaling_analysis.json")
            ...
            # Save visualization results
            if viz_results:
                analyzer.experiment_logger.save_results(viz_results, f"step14_scaling_visualization.json")
        ...
        if '15' not in args.skip_steps:
            ...
            # Save grokking results
            if grokking_results:
                analyzer.experiment_logger.save_results(grokking_results, f"step15_grokking_analysis_layer_{args.layer}.json")
        ...
        if '16' not in args.skip_steps:
            ...
            # Save sparse probing visualization results
            if probe_viz_results:
                analyzer.experiment_logger.save_results(probe_viz_results, f"step16_sparse_probing_visualization_layer_{args.layer}.json")
            ...
            if '17' not in args.skip_steps:
                ...
                # Save SPD results
                if spd_results:
                    analyzer.experiment_logger.save_results(spd_results, f"step17_spd_analysis_layer_{args.layer}.json")
                
                logger.info(f"SPD analysis complete for layer {args.layer}")
                if 'clusters' in spd_results and 'clusters' in spd_results['clusters']:
                    logger.info(f"Found {len(spd_results['clusters']['clusters'])} parameter clusters")
                if 'attributions_stats' in spd_results:
                    stats = spd_results['attributions_stats']
                    if 'total_attribution_norm' in stats:
                        logger.info(f"Attributions norm: {stats['total_attribution_norm']:.6f}")
                    if 'gradient_coverage' in stats:
                        logger.info(f"Gradient coverage: {stats['gradient_coverage']:.3f}")
                logger.info("✅ Stochastic Parameter Decomposition analysis complete!")
                
                # Step 17b: StableMamba Separate SPD Analysis
                logger.info("Step 17b: Running StableMamba separate SPD analysis...")
                
                try:
                    stable_mamba_spd_results = spd_analyzer.run_stable_mamba_separate_spd(
                        layer_idx=args.layer,
                        reference_text="The quick brown fox jumps over the lazy dog.",
                    ...
                    # Save StableMamba SPD results
                    if stable_mamba_spd_results:
                        analyzer.experiment_logger.save_results(stable_mamba_spd_results, f"step17b_stable_mamba_spd_analysis_layer_{args.layer}.json")
                    
                    if stable_mamba_spd_results:
                        if 'num_parameters' in stable_mamba_spd_results:
                            logger.info(f"StableMamba SPD complete: {stable_mamba_spd_results['num_parameters']} parameters analyzed")
                        if 'num_clusters' in stable_mamba_spd_results:
                            logger.info(f"Found {stable_mamba_spd_results['num_clusters']} StableMamba clusters")
                    logger.info("✅ StableMamba separate SPD analysis complete!")
                    
                except Exception as e:
                    logger.error(f"StableMamba SPD failed: {e}")
                    import traceback
                    logger.error(traceback.format_exc())
                
                # Step 17c: Enhanced StableMamba SPD Analysis
                logger.info("Step 17c: Running enhanced StableMamba SPD analysis...")
                
                try:
                    enhanced_stable_mamba_results = spd_analyzer.run_enhanced_stable_mamba_spd(
                        layer_idx=args.layer,
                        reference_text="The quick brown fox jumps over the lazy dog.",
                        n_samples=50,
                        sigma=1e-3
                    ...
                    # Save enhanced StableMamba SPD results
                    if enhanced_stable_mamba_results:
                        analyzer.experiment_logger.save_results(enhanced_stable_mamba_results, f"step17c_enhanced_stable_mamba_spd_layer_{args.layer}.json")
                    
                    logger.info(f"Enhanced StableMamba SPD complete!")
                    if enhanced_stable_mamba_results:
                        if 'num_parameters' in enhanced_stable_mamba_results:
                            logger.info(f"  Parameters analyzed: {enhanced_stable_mamba_results['num_parameters']}")
                        if 'gradient_coverage' in enhanced_stable_mamba_results:
                            logger.info(f"  Gradient coverage: {enhanced_stable_mamba_results['gradient_coverage']}")
                        if 'num_clusters' in enhanced_stable_mamba_results:
                            logger.info(f"  Clusters: {enhanced_stable_mamba_results['num_clusters']}")
                    logger.info("✅ Enhanced StableMamba SPD analysis complete!")
                    
                except Exception as e:
                    logger.error(f"Enhanced StableMamba SPD failed: {e}")
                    import traceback
                    logger.error(traceback.format_exc())
        ...
        if '18' not in args.skip_steps:
            ...
            # APD results are already saved in apd_analyzer.run(), but save main results here too
            analyzer.experiment_logger.save_results(apd_results, f"step18_apd_results_layer_{args.layer}.json")
            ...
            try:
                ...
                # Save layer transition results
                if layer_transition_results:
                    analyzer.experiment_logger.save_results(layer_transition_results, f"step18a_layer_transition_layer_{args.layer}.json")
            ...
            try:
                ...
                if spd_file.exists():
                    ...
                    # Save APD-SPD comparison results
                    if apd_spd_comparison:
                        analyzer.experiment_logger.save_results(apd_spd_comparison, f"step18b_apd_spd_consistency_layer_{args.layer}.json")
            ...
            try:
                ...
                # Save critical 5% results
                if critical_5_percent:
                    analyzer.experiment_logger.save_results(critical_5_percent, f"step18c_critical_5_percent_layer_{args.layer}.json")
            ...
            try:
                ...
                if apd_viz_path:
                    analyzer.experiment_logger.save_results({'apd_visualization_path': apd_viz_path}, f"step18d_apd_visualization_layer_{args.layer}.json")
            except Exception as e:
        ...
        if 'ssm' not in args.skip_steps:
            ...
            # Save SSM visualization results
            if viz_results:
                analyzer.experiment_logger.save_results(viz_results, f"ssm_visualization_layer_{args.layer}.json")
        ...
        if 'seq' not in args.skip_steps:
            ...
            # Save temporal visualization results
            if viz_results:
                analyzer.experiment_logger.save_results(viz_results, f"temporal_visualization_layer_{args.layer}.json")
        ...
        if 'transformer' not in args.skip_steps:
            ...
            # Save transformer comparison results
            if comparison_results:
                analyzer.experiment_logger.save_results(comparison_results, f"transformer_comparison_layer_{args.layer}.json")
        ...
        if 'patching' not in args.skip_steps:
            ...
            # Save patching strategies results
            if patching_results:
                analyzer.experiment_logger.save_results(patching_results, f"patching_strategies_layer_{args.layer}.json")
        ...
        # Save comprehensive report
        if report:
            analyzer.experiment_logger.save_results(report, "comprehensive_report.json")
        
        # Run StableMamba analysis only if StableMamba mode is enabled
        # Check if StableMamba is explicitly skipped or if we're not in StableMamba mode
        should_run_stable_mamba = args.stablemamba
        stable_mamba_skip_steps = ['stable_mamba', 'visualizations', 'comprehensive']
        should_skip_stable_mamba_entirely = any(step in args.skip_steps for step in stable_mamba_skip_steps)
        
        if should_run_stable_mamba and not should_skip_stable_mamba_entirely:
            logger.info("Running StableMamba analysis for the same steps as regular Mamba...")
            try:
                # Run StableMamba analysis with the same skip_steps as regular analysis
                analyzer.run_stable_mamba_analysis_with_skip_steps(args.skip_steps)
                logger.info("✅ StableMamba analysis complete!")
            except Exception as e:
                logger.warning(f"StableMamba analysis failed: {e}")
        elif not should_run_stable_mamba:
            logger.debug("Skipping StableMamba analysis - not in StableMamba mode")
        else:
            logger.info("Skipping StableMamba analysis entirely due to skip_steps argument")
