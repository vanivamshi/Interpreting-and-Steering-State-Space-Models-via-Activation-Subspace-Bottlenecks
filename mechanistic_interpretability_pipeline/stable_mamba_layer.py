# COMMENTED: Original StableMamba implementation with enhanced features
...
# COMMENTED: Enhanced StableMambaLayer implementation
# This implementation includes:
# - Enhanced SSMBlock with more parameters (5 gates, 32-state SSM)
# - Multiple scaling parameters for gradient monitoring
# - Pre/post normalization layers
...
# class EnhancedStableMambaLayer(nn.Module):
#     """
#     Enhanced StableMambaLayer with more parameters for increased influence.
#     """
...
def replace_layer_with_enhanced_stable_mamba(model, layer_idx: int, d_model: int = 768):
    """
    FIXED: Properly replace layer with enhanced StableMamba
    """
    logger.info(f"Replacing layer {layer_idx} with FIXED StableMamba...")
    
    # Create final fixed StableMamba layer
    stable_mamba_layer = StableMambaLayer(d_model=d_model, n_gates=3, n_timescales=3, d_state=16)
    ...
    stable_mamba_layer.apply(init_weights)
    
    # Move to same device and dtype as the model
    stable_mamba_layer = stable_mamba_layer.to(device=device, dtype=dtype)
    ...
    for name, buffer in stable_mamba_layer.named_buffers():
    ...
    # ✅ FIX: Create wrapper that ADDS StableMamba, doesn't replace
    class StableMambaWrappedLayer(nn.Module):
        def __init__(self, original, stable_mamba, layer_idx):
            super().__init__()
            self.original = original
            self.stable_mamba = stable_mamba
            self.layer_idx = layer_idx
            self.norm = original.norm
            
            # ✅ ADD: Learnable mixing weight
            self.mix_weight = nn.Parameter(torch.tensor(0.1))  # Start with 10% StableMamba
        ...
        def forward(self, hidden_states, residual=None, inference_params=None, cache_params=None, **kwargs):
            ...
            stable_mamba_out = self.stable_mamba(hidden_states, self.layer_idx)
            
            if squeeze_output:
                stable_mamba_out = stable_mamba_out.squeeze(0)
            
            # ✅ FIX: Gradually mix in StableMamba
            mix_weight_sigmoid = torch.sigmoid(self.mix_weight)
            output = original_out * (1 - mix_weight_sigmoid) + stable_mamba_out * mix_weight_sigmoid
            
            return output
    
    # Replace the layer
    wrapped = StableMambaWrappedLayer(original_layer, stable_mamba_layer, layer_idx)
    model.backbone.layers[layer_idx] = wrapped
    
    logger.info(f"✅ Layer {layer_idx} now uses FIXED StableMamba with gradual mixing")
    return stable_mamba_layer

def attach_stable_mamba_layers(model: nn.Module, attribute_name: str = "stable_mamba") -> int:
    """
    FIXED: Attach StableMamba layers with proper integration
    """
    logger.info("Attaching enhanced StableMamba layers (using working implementation)...")
    ...
    for idx, layer in enumerate(layers):
        ...
        if idx < 3:  # Only attach to first 3 layers for stability
            try:
                stable_mamba_layer = replace_layer_with_enhanced_stable_mamba(model, idx, d_model)
                count += 1
                logger.info(f"✅ Enhanced StableMamba attached to layer {idx}")
            except Exception as e:
                logger.warning(f"Failed to attach enhanced StableMamba to layer {idx}: {e}")
                # Fallback to simple attachment
                stable_mamba_layer = StableMambaLayer(d_model)
                device = next(model.parameters()).device
                dtype = next(model.parameters()).dtype
                stable_mamba_layer = stable_mamba_layer.to(device=device, dtype=dtype)
                ...
                setattr(layer, attribute_name, stable_mamba_layer)
                count += 1
        else:
    ...
    logger.info(f"✅ Attached StableMamba layers to {count} layers")
    return count


def test_stable_mamba_gradient_flow():
    """Test that gradients actually flow through StableMamba"""
    stable_mamba = StableMambaLayer(d_model=768, n_gates=3, d_state=16)
    
    x = torch.randn(2, 10, 768, requires_grad=True)
    output = stable_mamba(x)
    ...
    for param_name in key_params:
        param = dict(stable_mamba.named_parameters())[param_name]
...
if __name__ == "__main__":
    ...
    test_stable_mamba_gradient_flow()
    
    # Minimal sanity test when run directly
    torch.manual_seed(0)
    d_model = 64
    layer = StableMambaLayer(d_model)
...
def identify_stable_mamba_circuits(self, layer_idx: int = 0) -> Dict[str, Any]:
    """
    Identify circuit candidates specifically for StableMamba architecture.
    
    StableMamba has distinct components that form circuits:
    ...
    """
    logger.info(f"Step 6a: Identifying StableMamba-specific circuits for layer {layer_idx}...")
    ...
    activation_data = self.stable_mamba_activation_data[layer_idx]
    circuits = []
    ...
    # Save Results
    # ========================================
    circuits_file = self.experiment_logger.experiment_dir / f"stable_mamba_candidate_circuits_layer_{layer_idx}.json"
    ...
    logger.info(f"✓ Identified {len(circuits)} StableMamba circuits for layer {layer_idx}")
    logger.info(f"  Circuit types: {', '.join(result['circuit_types'])}")
    
    return result


def add_stable_mamba_activation_hooks(analyzer, model, layer_idx: int = 0):
    """
    Add hooks to capture StableMamba-specific activations for circuit analysis.
    Must be called before running forward passes.
    """
    if not hasattr(analyzer, 'stable_mamba_activation_data'):
        analyzer.stable_mamba_activation_data = {}
    
    analyzer.stable_mamba_activation_data[layer_idx] = {}
    ...
    stable_mamba = layer.stable_mamba
    
    # Hook for each gate in ensemble
    for i, gate in enumerate(stable_mamba.gates):
        def gate_hook(module, input, output, gate_idx=i):
            analyzer.stable_mamba_activation_data[layer_idx][f'gate_{gate_idx}'] = output.detach().cpu().numpy()
        gate.register_forward_hook(gate_hook)
    
    # Hook for each timescale SSM
    for name in ['ssm_fast', 'ssm_medium', 'ssm_slow']:
        ssm = getattr(stable_mamba, name)
        def ssm_hook(module, input, output, ssm_name=name):
            analyzer.stable_mamba_activation_data[layer_idx][ssm_name] = output.detach().cpu().numpy()
        ssm.register_forward_hook(ssm_hook)
    
    # Hook for sparse attention (if applicable)
    def attn_hook(module, input, output):
        analyzer.stable_mamba_activation_data[layer_idx]['sparse_attn'] = output.detach().cpu().numpy()
    stable_mamba.sparse_attn.register_forward_hook(attn_hook)
    
    # Hook for memory gate
    def mem_gate_hook(module, input, output):
        analyzer.stable_mamba_activation_data[layer_idx]['memory_gate'] = output.detach().cpu().numpy()
    stable_mamba.memory_gate.register_forward_hook(mem_gate_hook)
    
    # Hook for compression predictor
    def comp_hook(module, input, output):
        analyzer.stable_mamba_activation_data[layer_idx]['compression'] = output.detach().cpu().numpy()
    stable_mamba.compression_predictor.register_forward_hook(comp_hook)
    
    logger.info(f"✓ Added StableMamba activation hooks for layer {layer_idx}")


# ============================================================================
# FINAL FIXED StableMamba Implementation - All gradient flow issues resolved
# ============================================================================
...
class StableMambaLayer(nn.Module):
    """
    FINAL FIXED StableMamba Layer with all gradient issues resolved
    """
...
def replace_layer_with_final_fixed_stable_mamba(model, layer_idx: int, d_model: int = 768):
    """
    FINAL FIXED: Replace layer with properly integrated StableMamba
    """
    import logging
    logger = logging.getLogger(__name__)
    
    logger.info(f"Replacing layer {layer_idx} with FINAL FIXED StableMamba...")
    
    # Create fixed StableMamba layer
    stable_mamba_layer = StableMambaLayer(d_model=d_model, n_gates=3, n_timescales=3, d_state=16)
    stable_mamba_layer = stable_mamba_layer.to(next(model.parameters()).device)
    stable_mamba_layer = stable_mamba_layer.to(next(model.parameters()).dtype)
    ...
    class StableMambaWrappedLayer(nn.Module):
        def __init__(self, original, stable_mamba, layer_idx):
            ...
            # ✅ FIXED: Stable mixing weight (start at 50% StableMamba)
        ...
        def forward(self, hidden_states, residual=None, inference_params=None, cache_params=None, cache_position=None, **kwargs):
            ...
            stable_mamba_out = self.stable_mamba(hidden_states_3d, self.layer_idx)
            ...
            output = original_out * (1 - mix_alpha) + stable_mamba_out * mix_alpha
    ...
    logger.info(f"✅ Layer {layer_idx} now uses FINAL FIXED StableMamba")
