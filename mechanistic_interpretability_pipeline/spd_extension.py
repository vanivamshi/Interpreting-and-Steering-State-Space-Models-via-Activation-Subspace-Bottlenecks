...
# ============================================================================
# ENHANCED STABLE_MAMBA IMPLEMENTATION FOR STEPS 17, 18, 19
# ============================================================================
...
class StableMambaLayer(nn.Module):
    """
    Enhanced StableMamba Layer with:
...
class SPDAnalyzer:
    ...
    def run_stable_mamba_separate_spd(self, layer_idx: int = 0, reference_text: str = "The quick brown fox jumps over the lazy dog.", n_samples: int = 50, sigma: float = 1e-3):
        """Run StableMamba separate SPD analysis using enhanced analyzer"""
        return self.enhanced_analyzer.run_enhanced_spd_analysis(
            layer_idx=layer_idx,
            reference_text=reference_text,
            n_attrib_steps=4,
    ...
    def run_enhanced_stable_mamba_spd(self, layer_idx: int = 0, reference_text: str = "The quick brown fox jumps over the lazy dog.", n_samples: int = 50, sigma: float = 1e-3):
        """Run enhanced StableMamba SPD analysis using enhanced analyzer"""
        return self.enhanced_analyzer.run_enhanced_spd_analysis(
            layer_idx=layer_idx,
            reference_text=reference_text,
            n_attrib_steps=4,
...
def replace_layer_with_enhanced_stable_mamba(model, layer_idx: int, d_model: int = 768):
    """
    Replace a model layer's forward pass with enhanced StableMamba
    
    This creates a new StableMambaLayer and wraps the original layer's forward
    to route through the enhanced architecture.
    """
    logger.info(f"Replacing layer {layer_idx} with enhanced StableMamba...")
    
    # Create enhanced StableMamba layer
    ...
    # Create wrapper that routes through StableMamba
    class StableMambaWrappedLayer(nn.Module):
        ...
        def forward(self, hidden_states, residual=None, inference_params=None):
            # Route through enhanced StableMamba instead of original mixer
            normed = self.norm(hidden_states)
            
            # Enhanced StableMamba forward (handles 2D or 3D input)
            if normed.dim() == 2:
            ...
            stable_mamba_out = self.stable_mamba(normed, self.layer_idx)
            ...
            if residual is not None:
                output = residual + stable_mamba_out
            else:
                output = hidden_states + stable_mamba_out
    ...
    logger.info(f"✅ Layer {layer_idx} now uses enhanced StableMamba")
