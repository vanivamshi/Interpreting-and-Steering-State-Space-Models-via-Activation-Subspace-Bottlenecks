...
class StateInterpolationAnalyzer:
    ...
    def visualize_state_interpolation(self, analysis_results: Dict, neuron_indices: List[int], layer_idx: int = 0, averaged: bool = False):
        ...
        if conflict_types:
            ...
            ax1.set_xlabel('Conflict Type', fontsize=18, fontweight='bold')
            ax1.set_ylabel('Metric Value', fontsize=18, fontweight='bold')
            title_suffix = " (Averaged Across All Layers)" if averaged else ""
            ax1.set_title('Interpolation Metrics by Conflict Type' + title_suffix, fontsize=20, fontweight='bold')
            ax1.set_xticks(x)
            ax1.set_xticklabels(conflict_types, rotation=45, ha='right', fontsize=16)
        ...
        if all_patterns:
            ...
            ax2.set_title('Resolution Pattern Distribution', fontsize=20, fontweight='bold')
            for text in texts:
        ...
        if curvature_by_type:
            ...
            ax3.set_xlabel('Average Trajectory Curvature', fontsize=18, fontweight='bold')
            ax3.set_title('State Trajectory Curvature by Conflict Type', fontsize=20, fontweight='bold')
            ...
            for bar, value in zip(bars, curvature_by_type):
                ax3.text(bar.get_width() + 0.01, bar.get_y() + bar.get_height()/2, 
                        f'{value:.3f}', va='center', ha='left', fontsize=16)
        ...
        if averaged:
            plt.savefig(f"images/state_interpolation_analysis.png", dpi=300, bbox_inches='tight')
            print(f"✅ Saved averaged state interpolation analysis plot to images/state_interpolation_analysis.png")
        else:
            plt.savefig(f"images/state_interpolation_analysis_layer{layer_idx}.png", dpi=300, bbox_inches='tight')
            print(f"✅ Saved state interpolation analysis plot to images/state_interpolation_analysis_layer{layer_idx}.png")
        plt.close()
...
def run_state_interpolation_analysis():
    ...
    fig = analyzer.visualize_state_interpolation(results, target_neurons, layer_idx=layer_idx)
