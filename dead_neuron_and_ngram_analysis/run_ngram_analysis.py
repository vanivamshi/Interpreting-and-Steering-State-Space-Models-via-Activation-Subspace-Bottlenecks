# Run as: python3 run_ngram_analysis.py
"""
...
def plot_paper_figure2(all_layer_triggers, all_dead_neurons, model_label, save_dir="plots"):
    ...
    # Increase font sizes for paper visibility (larger for compressed figures)
    ax.set_xlabel("Layer (relative depth)", fontsize=20, fontweight='bold')
    ax.set_ylabel("Number of Neurons", fontsize=20, fontweight='bold')
    ax.set_title(f"Neurons Categorised by Unigram Triggers in {model_label}", fontsize=22, fontweight='bold')
    ax.legend(title="Unigrams triggering neuron", bbox_to_anchor=(1.05,1), loc="upper left", 
              fontsize=16, title_fontsize=18, prop={'weight': 'bold'})
    # Increase tick label sizes
    ax.tick_params(axis='both', which='major', labelsize=18, width=2, length=5)
...
def plot_overlay_figures(all_results, save_dir="plots"):
    ...
    for model_label,res in all_results.items():
        ...
        plt.plot(depths, per_layer_neurons, marker="o", label=model_label, linewidth=3.5, markersize=10)
    plt.xlabel("Layer (Relative Depth)", fontsize=20, fontweight='bold')
    plt.ylabel("Number of Neurons", fontsize=20, fontweight='bold')
    plt.title("Token-detecting neurons", fontsize=20, fontweight='bold')
    plt.grid(True,alpha=0.3)
    plt.legend(fontsize=16, prop={'weight': 'bold'})
    plt.tick_params(axis='both', which='major', labelsize=16, width=2, length=5)
    ...
    for model_label,res in all_results.items():
        ...
        plt.plot(depths, cumulative_tokens, linestyle="-", label=f"{model_label}", linewidth=3.5)
    plt.xlabel("Layer (Relative Depth)", fontsize=20, fontweight='bold')
    plt.ylabel("Number of Tokens Covered", fontsize=20, fontweight='bold')
    plt.title("Token coverage", fontsize=20, fontweight='bold')
    plt.grid(True,alpha=0.3)
    plt.legend(fontsize=16, prop={'weight': 'bold'})
    plt.tick_params(axis='both', which='major', labelsize=16, width=2, length=5)
    plt.tick_params(axis='both', which='minor', labelsize=16)
    ...
    for model_label,res in all_results.items():
        ...
        plt.plot(depths, new_overall, marker="o", label=f"{model_label}", linewidth=3.5, markersize=10)
        # plt.plot(depths, new_vs_prev, marker="s", linestyle="--", label=f"{model_label} new vs prev")  # Commented out: removed new vs prev plot
    plt.xlabel("Layer (Relative Depth)", fontsize=20, fontweight='bold')
    plt.ylabel("Number of New Tokens", fontsize=20, fontweight='bold')
    plt.title("Number of Covered New Tokens", fontsize=20, fontweight='bold')
    plt.grid(True,alpha=0.3)
    plt.legend(fontsize=16, prop={'weight': 'bold'})
    plt.tick_params(axis='both', which='major', labelsize=16, width=2, length=5)
    plt.tick_params(axis='both', which='minor', labelsize=16)
...
if __name__=="__main__":
    ...
    for model_label,model_name in models_to_analyze.items():
        try:
            ...
            print(f"✅ Successfully completed analysis for {model_label}")
        except Exception as e:
            print(f"❌ Error analyzing {model_label}: {e}")
            import traceback
            traceback.print_exc()
            print(f"   Skipping {model_label} and continuing with remaining models...")
    ...
    # Print summary of successfully analyzed models
    print(f"\n📊 Summary: Successfully analyzed {len(all_results)}/{len(models_to_analyze)} models")
    print(f"   Models analyzed: {', '.join(sorted(all_results.keys()))}")
    if len(all_results) < len(models_to_analyze):
        missing = set(models_to_analyze.keys()) - set(all_results.keys())
        print(f"   ⚠️  Models that failed: {', '.join(sorted(missing))}")
    
    if len(all_results) == 0:
        print("\n❌ No models were successfully analyzed. Cannot generate plots.")
        sys.exit(1)
    ...
    print(f"\n🎉 Multi-model N-gram analysis complete!")
    print(f"   ✅ {len(all_results)} models saved to log file")
    print(f"   ✅ Paper-style Figures 2–4 saved in 'plots/'")