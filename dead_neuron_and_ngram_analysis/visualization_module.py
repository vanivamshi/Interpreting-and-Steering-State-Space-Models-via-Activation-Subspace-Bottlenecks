...
def plot_dead_neuron_distribution(all_models_data, save_path=None):
    ...
    plt.xlabel("Layer", fontsize=20, fontweight='bold')
    plt.ylabel("Fraction of Dead Neurons", fontsize=20, fontweight='bold')
    plt.title("Dead Neuron Distribution Across Layers", fontsize=22, fontweight='bold')
    plt.legend(fontsize=16, title_fontsize=18, prop={'weight': 'bold'})
    plt.grid(True)
...
def plot_dead_neuron_stats_by_layer(layer_results, model_name, save_dir="plots"):
    ...
    plt.xlabel("Layer (relative depth)", fontsize=20, fontweight='bold')
    plt.ylabel("% Dead Neurons", fontsize=20, fontweight='bold')
    plt.title(f"(a) % Dead Neurons Across Layers\n{model_name}", fontsize=22, fontweight='bold')
    ...
    plt.xlabel("Layer (relative depth)", fontsize=20, fontweight='bold')
    plt.ylabel("Avg Activation Frequency (non-dead)", fontsize=20, fontweight='bold')
...
def plot_neuron_distribution(dead_freq, pos_corr, model_name, layer_idx, save_dir="plots"):
    ...
    plt.title(f'Dead Neuron Activation Distribution\n{model_name} Layer {layer_idx}', fontsize=22, fontweight='bold')
    plt.xlabel('Maximum Activation Value', fontsize=20, fontweight='bold')
    plt.ylabel('Number of Neurons', fontsize=20, fontweight='bold')
    ...
    plt.xlabel('Correlation with Token Position', fontsize=20, fontweight='bold')
    plt.ylabel('Number of Neurons', fontsize=20, fontweight='bold')
...
def plot_comprehensive_ablation(ablation_results, model_name, layer_idx, save_dir="plots"):
    ...
    for bar, ppl in zip(bars1, perplexities):
        plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + max(perplexities) * 0.01,
                f'{ppl:.2f}', ha='center', va='bottom', fontweight='bold', fontsize=16)
    
    plt.xticks(range(len(conditions)), readable_labels, fontsize=18)
    ...
    for bar, pct in zip(bars2, pct_changes):
        plt.text(bar.get_x() + bar.get_width()/2, 
                ...
                f'{pct:+.1f}%', ha='center', va='bottom' if pct >= 0 else 'top', fontweight='bold', fontsize=16)
    
    plt.xticks(range(len(conditions)), readable_labels, fontsize=18)
    plt.ylabel('Perplexity Change (%)', fontsize=20, fontweight='bold')
    plt.title('Percentage Change from Baseline', fontsize=22, fontweight='bold')
...
def plot_dead_neuron_stats(dead_neurons, dead_frequencies, model_name, layer_idx, save_dir="plots"):
    ...
    plt.ylabel("Average Max Activation", fontsize=20, fontweight='bold')
...
def plot_positional_neurons_scatter(positional_correlations, model_name, layer_idx, save_dir="plots"):
    ...
    plt.axhline(y=-0.3, color="gray", linestyle="--", alpha=0.7)
    plt.xlabel("Neuron Index", fontsize=20, fontweight='bold')
    plt.ylabel("Correlation with Token Position", fontsize=20, fontweight='bold')
...
def plot_neuron_activation_heatmap(activations_matrix, tokens, neuron_indices, model_label, layer_idx, sample_text_idx, save_dir="plots"):
    ...
    plt.ylabel("Neuron Index", fontsize=20, fontweight='bold')
    plt.title(f"Neuron Activations Heatmap\n{model_label} Layer {layer_idx} (Sample Text {sample_text_idx})", fontsize=22, fontweight='bold')
...
def plot_pruning_results(pruning_results, model_name, layer_idx, save_dir="plots"):
    ...
    for bar, ppl in zip(bars, perplexities):
        plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + max(perplexities) * 0.01,
                f'{ppl:.3f}', ha='center', va='bottom', fontweight='bold', fontsize=16)
    ...
    plt.title(f'Neuron Pruning Distribution\n({pruning_results["pruning_ratio"]:.1%} pruned)', fontsize=22, fontweight='bold')
    plt.tight_layout()
...
def plot_attention_neurons_analysis(attention_data, model_name, layer_idx, save_dir="plots"):
    ...
    fig.suptitle(f'Attention Neurons Analysis - {model_name} (Layer {layer_idx})', fontsize=22, fontweight='bold')
    ...
    try:
        ...
        if 'analysis_results' in attention_data and 'attention_weighted' in attention_data['analysis_results']:
            ...
            if 'neuron_activations' in layer_data:
                ...
                axes[0, 0].set_title('Neuron Activation Distribution', fontsize=18, fontweight='bold')
                axes[0, 0].set_xlabel('Activation Value', fontsize=16, fontweight='bold')
                axes[0, 0].set_ylabel('Frequency', fontsize=16, fontweight='bold')
                ...
                axes[0, 1].set_title(f'Top {top_k} Neurons by Activation', fontsize=18, fontweight='bold')
                axes[0, 1].set_xlabel('Neuron Rank', fontsize=16, fontweight='bold')
                axes[0, 1].set_ylabel('Activation Value', fontsize=16, fontweight='bold')
                axes[0, 1].set_xticks(range(top_k))
                axes[0, 1].set_xticklabels([f'#{idx}' for idx in top_indices], rotation=45, fontsize=12)
                ...
                if 'neuron_importance' in layer_data:
                    ...
                    axes[0, 2].set_title('Activation vs Importance', fontsize=18, fontweight='bold')
                    axes[0, 2].set_xlabel('Activation Value', fontsize=16, fontweight='bold')
                    axes[0, 2].set_ylabel('Importance Score', fontsize=16, fontweight='bold')
                ...
                if 'attention_vectors' in attention_data.get('attention_data', {}).get(layer_idx, {}):
                    ...
                    axes[1, 0].set_title('Attention Heatmap (Layer Average)', fontsize=18, fontweight='bold')
                    axes[1, 0].set_xlabel('Sequence Position', fontsize=16, fontweight='bold')
                    axes[1, 0].set_ylabel('Neuron Index', fontsize=16, fontweight='bold')
                ...
                if 'neuron_diversity' in layer_data:
                    ...
                    axes[1, 1].text(0.5, 0.5, f'Neuron Diversity:\n{diversity:.4f}', 
                                   ...
                                   fontsize=18, fontweight='bold', bbox=dict(boxstyle="round,pad=0.3", facecolor="lightyellow"))
                    axes[1, 1].set_title('Neuron Diversity Score', fontsize=18, fontweight='bold')
                    axes[1, 1].axis('off')
                ...
                axes[1, 2].set_title('Statistical Summary', fontsize=18, fontweight='bold')
                axes[1, 2].axis('off')
            ...
            for ax in axes.flat:
                ...
                ax.set_title('Data Not Available', fontsize=18, fontweight='bold')
                ax.axis('off')
        ...
        for ax in axes.flat:
            ...
            ax.set_title('Visualization Error', fontsize=18, fontweight='bold')
            ax.axis('off')
