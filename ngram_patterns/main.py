...
def plot_ablation_results(ablation_results, model_name, layer_idx):
    ...
    plt.xticks(range(len(conditions)), readable_labels, rotation=45, ha='right', fontsize=16)
...
def run_comprehensive_analysis(model, tokenizer, texts, layer_idx=0, top_k=10, model_name="model"):
    ...
    results = {
        'model_config': {
            ...
            'model_type': 'Mamba' if 'mamba' in model_name.lower() else 'Transformer'
        },
        'analysis_results': {}
    }
    ...
    # Run attention neurons analysis (with Mamba-specific handling)
    ...
    try:
        ...
        # Use Mamba-specific attention analysis if it's a Mamba model
        if 'mamba' in model_name.lower():
            ...
            # For Transformer models, use the original method
            attention_neurons = integrate_mamba_attention_neurons(
                model, sample_input, layer_indices=[layer_idx], methods=['attention_weighted']
            )
