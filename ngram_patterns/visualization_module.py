...
def plot_dead_neuron_distribution(all_models_data, save_path=None):
    ...
    plt.ylabel("Fraction of Dead Neurons", fontsize=18)
    plt.title("Dead Neuron Distribution Across Layers", fontsize=20, fontweight='bold')
...
def plot_dead_neuron_stats_by_layer(layer_results, model_name, save_dir="plots"):
    ...
    plt.xlabel("Layer (relative depth)", fontsize=18)
    plt.ylabel("% Dead Neurons", fontsize=18)
    ...
    plt.xlabel("Layer (relative depth)", fontsize=18)
    plt.ylabel("Avg Activation Frequency (non-dead)", fontsize=18)
...
def plot_neuron_distribution(dead_freq, pos_corr, model_name, layer_idx, save_dir="plots"):
    ...
    plt.title(f'Dead Neuron Activation Distribution\n{model_name} Layer {layer_idx}', fontsize=20, fontweight='bold')
    plt.xlabel('Maximum Activation Value', fontsize=18)
    ...
    plt.xlabel('Correlation with Token Position', fontsize=18)
    plt.ylabel('Number of Neurons', fontsize=18)
    plt.tick_params(labelsize=16)
...
def plot_comprehensive_ablation(ablation_results, model_name, layer_idx, save_dir="plots"):
    ...
    plt.xticks(range(len(conditions)), readable_labels, fontsize=16)
    ...
    plt.xticks(range(len(conditions)), readable_labels, fontsize=16)
    plt.ylabel('Perplexity Change (%)', fontsize=18)
    plt.title('Percentage Change from Baseline', fontsize=20, fontweight='bold')
    plt.tick_params(labelsize=16)
...
def plot_dead_neuron_stats(dead_neurons, dead_frequencies, model_name, layer_idx, save_dir="plots"):
    ...
    plt.ylabel("Average Max Activation", fontsize=18)
...
def plot_positional_neurons_scatter(positional_correlations, model_name, layer_idx, save_dir="plots"):
    ...
    plt.ylabel("Correlation with Token Position", fontsize=18)
    plt.title(f"Positional Neurons\n{model_name} Layer {layer_idx}", fontsize=20, fontweight='bold')
    plt.tick_params(labelsize=16)
    cbar = plt.colorbar(scatter, label="Correlation Strength")
    cbar.set_label("Correlation Strength", fontsize=18)
...
def plot_multiple_positional_neurons_scatter(all_models_correlations, layer_identifier, save_dir="plots"):
    ...
    for i, (plot_label, correlations) in enumerate(all_models_correlations.items()):
        ...
        ax.axhline(y=-0.3, color="gray", linestyle="--", alpha=0.7)
        ax.set_xlabel("Neuron Index", fontsize=18)
        ax.set_ylabel("Correlation", fontsize=18)
        ax.set_title(f"{plot_label}", fontsize=20, fontweight='bold')
    ...
    plt.suptitle(f"Positional Neurons Across Models)", fontsize=20, fontweight='bold', y=1.02)
...
def plot_positional_ablation_results_multi_model(all_ablation_results, layer_identifier, save_dir="plots"):
    ...
    for i, (plot_label, results) in enumerate(all_ablation_results.items()):
        ...
        ax.set_title(f"{plot_label}\n({num_ablated} neurons ablated)", fontsize=20, fontweight='bold')
        ax.tick_params(labelsize=16)
    ...
    plt.suptitle(f"Positional Neuron Ablation Study (Layer {layer_identifier})", fontsize=20, fontweight='bold', y=1.02)
...
def plot_neuron_activation_heatmap(activations_matrix, tokens, neuron_indices, model_label, layer_idx, sample_text_idx, save_dir="plots"):
    ...
    ax = sns.heatmap(activations_matrix.T, # Transpose to have neurons on Y-axis, tokens on X-axis
...
def plot_pruning_results(pruning_results, model_name, layer_idx, save_dir="plots"):
    ...
    plt.title(f'Neuron Pruning Distribution\n({pruning_results["pruning_ratio"]:.1%} pruned)', fontsize=20, fontweight='bold')
    plt.tight_layout()
...
def plot_attention_neurons_analysis(attention_data, model_name, layer_idx, save_dir="plots"):
    ...
    fig.suptitle(f'Attention Neurons Analysis - {model_name} (Layer {layer_idx})', fontsize=20, fontweight='bold')
    ...
    try:
        ...
        if 'analysis_results' in attention_data and 'attention_weighted' in attention_data['analysis_results']:
            ...
            if 'neuron_activations' in layer_data:
                ...
                axes[0, 0].set_title('Neuron Activation Distribution', fontsize=20, fontweight='bold')
                axes[0, 0].set_xlabel('Activation Value', fontsize=18)
                ...
                axes[0, 1].set_title(f'Top {top_k} Neurons by Activation', fontsize=20, fontweight='bold')
                axes[0, 1].set_xlabel('Neuron Rank', fontsize=18)
                axes[0, 1].set_ylabel('Activation Value', fontsize=18)
                axes[0, 1].set_xticks(range(top_k))
                axes[0, 1].set_xticklabels([f'#{idx}' for idx in top_indices], rotation=45, fontsize=16)
                ...
                if 'neuron_importance' in layer_data:
                    ...
                    axes[0, 2].set_title('Activation vs Importance', fontsize=20, fontweight='bold')
                    axes[0, 2].set_xlabel('Activation Value', fontsize=18)
                    axes[0, 2].set_ylabel('Importance Score', fontsize=18)
                ...
                if 'attention_vectors' in attention_data.get('attention_data', {}).get(layer_idx, {}):
                    ...
                    axes[1, 0].set_title('Attention Heatmap (Layer Average)', fontsize=20, fontweight='bold')
                ...
                if 'neuron_diversity' in layer_data:
                    ...
                    axes[1, 1].set_title('Neuron Diversity Score', fontsize=20, fontweight='bold')
                    axes[1, 1].axis('off')
                ...
                axes[1, 2].set_title('Statistical Summary', fontsize=20, fontweight='bold')
                axes[1, 2].axis('off')
