    ...
    for dim in test_dims:
        ...
        try:
            ...
            if scores:
                ...
                plt.title(f"Inter-layer Causal Impact - Dimension {dim}", fontsize=20, fontweight='bold')
                plt.xlabel("Layer Index", fontsize=18)
                plt.ylabel("Impact Score (KL Divergence)", fontsize=18)
...
def run_cross_layer_analysis(model, tokenizer):
    ...
    for dim in test_dims:
        print(f"\n🔬 Analyzing dimension {dim}...")
        try:
            # Create matrix to store cross-layer influences
            influence_matrix = np.zeros((max_layers_to_test, max_layers_to_test))
            
            # Test cross-layer influence between all layer pairs
            for src_layer in range(max_layers_to_test):
                for tgt_layer in range(max_layers_to_test):
                    ...
                    if result is not None:
                        influence_matrix[src_layer, tgt_layer] = result
                        successful_analyses += 1
                        print(f"  ✅ L{src_layer}→L{tgt_layer} dim{dim}: {result:.6f}")
                    else:
                        print(f"  ❌ L{src_layer}→L{tgt_layer} dim{dim}: No result")
            
            # Create cross-layer influence matrix heatmap
            if successful_analyses > 0:
                print(f"\n🎨 Creating cross-layer influence matrix for dimension {dim}...")
                
                plt.figure(figsize=(10, 8))
                
                # Create heatmap
                im = plt.imshow(influence_matrix, cmap='viridis', aspect='auto')
                
                # Set axis labels as requested
                plt.xlabel('Target Layer Activation', fontsize=30)
                plt.ylabel('Source Layer Activation', fontsize=30)
                plt.title(f'Cross-Layer Causal Influence Matrix (Dim {dim})', fontsize=20, fontweight='bold', pad=20)
                
                # Set tick labels
                plt.xticks(range(max_layers_to_test), range(max_layers_to_test))
                plt.yticks(range(max_layers_to_test), range(max_layers_to_test))
                plt.tick_params(labelsize=30)
                
                # Add colorbar
                cbar = plt.colorbar(im)
                cbar.set_label('Target Neuron Activation', fontsize=30)
                cbar.ax.tick_params(labelsize=30)
                
                # Add text annotations to cells in red
                for i in range(max_layers_to_test):
                    for j in range(max_layers_to_test):
                        text = plt.text(j, i, f'{influence_matrix[i, j]:.3f}',
                                      ha="center", va="center", color="red", fontsize=24, fontweight='bold')
                
                plt.tight_layout()
                ensure_plot_display(f"Cross_Layer_Influence_Matrix_Dim_{dim}")
                print(f"  ✅ Successfully created cross-layer influence matrix for dimension {dim}")
...
def run_attention_neurons_analysis(model, tokenizer, texts, layer_idx=0, methods=None, plots_dir='plots'):
    ...
    try:
        ...
        print(f"🔍 Analyzing attention neurons for all layers...")
        print(f"📝 Sample text: '{sample_text[:100]}{'...' if len(sample_text) > 100 else ''}'")
        print(f"🔧 Using methods: {', '.join(methods)}")
        
        # Run attention neurons analysis with all layers (layer_indices=None extracts all)
        attention_results = integrate_mamba_attention_neurons(
            model, sample_input, 
            layer_indices=None,  # None means extract all layers
            methods=methods
        )
        ...
        if attention_results and 'analysis_results' in attention_results:
            ...
            if 'analyzer' in attention_results:
                ...
                try:
                    # Get mamba neurons for visualization (all layers)
                    if 'mamba_neurons' in attention_results and 'attention_weighted' in attention_results['mamba_neurons']:
                        ...
                        if mamba_neurons:
                            print(f"\n🎨 Generating attention neurons visualization for all layers...")
                            save_path = os.path.join(plots_dir, "mamba_all_layers.png")
                            analyzer.visualize_neurons(
                                mamba_neurons, 
                                layer_idx=None,  # None means plot all layers
                                save_path=save_path
                            )
...
def run_comparison_analysis(texts):
    ...
    try:
        # Run quick comparison first (creates Quick_Comparison_Summary.png)
        print("🔄 Running quick comparison analysis...")
        run_quick_comparison(texts)
        
        print("🔄 Loading models for full comparison...")
        models = load_models()
...
def analyze_model_efficiency(model, tokenizer, texts):
    ...
    try:
        ...
        if hasattr(outputs, 'hidden_states') and outputs.hidden_states:
            ...
            plt.title("Activation Distribution", fontsize=20, fontweight='bold')
            ...
            plt.title("Model Efficiency Metrics", fontsize=20, fontweight='bold')
            ...
            plt.title("Parameter Utilization", fontsize=20, fontweight='bold')
