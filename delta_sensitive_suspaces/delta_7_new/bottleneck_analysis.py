...
class BottleneckAnalyzer:
    ...
    def visualize_analysis(self, analysis_results: Dict, classification: Dict, layer_idx: int = 0, averaged: bool = False):
        ...
        ax1.set_xlabel('Perturbation Type', fontsize=18, fontweight='bold')
        ax1.set_ylabel('Effect Size\n(Relative PPL Change)', fontsize=18, fontweight='bold')
        title_suffix = " (Averaged Across All Layers)" if averaged else f" (Layer {layer_idx})"
        ax1.set_title('Perturbation Effects' + title_suffix, fontsize=20, fontweight='bold')
        ax1.set_xticks(range(len(labels)))
        ax1.set_xticklabels(labels, rotation=45, fontsize=16)
        ...
        if analysis_results['individual_effects']:
            ...
            ax2.set_xlabel('Neuron Index', fontsize=18, fontweight='bold')
            ax2.set_ylabel('Individual Effect Size', fontsize=18, fontweight='bold')
            ax2.set_title('Individual Neuron Contributions' + title_suffix, fontsize=20, fontweight='bold')
            ax2.set_xticks(range(len(neurons)))
            ax2.set_xticklabels([str(n) for n in neurons], fontsize=16)
        ...
        ax3.set_xlabel('Noise Standard Deviation', fontsize=18, fontweight='bold')
        ax3.set_ylabel('Effect Size', fontsize=18, fontweight='bold')
        ax3.set_title('Noise Sensitivity Curve' + title_suffix, fontsize=20, fontweight='bold')
        ...
        ax4.set_ylim(0, 1)
        ax4.axis('off')
        ax4.set_title('Classification Results', fontsize=20, fontweight='bold')
        ...
        if averaged:
            plt.savefig(f"images/bottleneck_analysis.png", dpi=300, bbox_inches='tight')
            print(f"✅ Saved averaged bottleneck analysis plot to images/bottleneck_analysis.png")
        else:
            plt.savefig(f"images/bottleneck_analysis_layer{layer_idx}.png", dpi=300, bbox_inches='tight')
            print(f"✅ Saved bottleneck analysis plot to images/bottleneck_analysis_layer{layer_idx}.png")
        plt.close()
        
        return fig


def average_bottleneck_results(all_layer_results: List[Dict]) -> Optional[Dict]:
    """Average bottleneck analysis results across all layers per neuron."""
    if not all_layer_results:
        return None
    
    # Collect all unique neurons across all layers
    all_neurons = set()
    for result in all_layer_results:
        if result and 'individual_effects' in result:
            all_neurons.update(result['individual_effects'].keys())
    ...
    # Average per neuron
    averaged = {
        'zero_perturbation': {'effect_size': 0.0},
        'mean_perturbation': {'effect_size': 0.0},
        'noise_perturbations': {},
        'individual_effects': {},
        'baseline_ppl': 0.0
    }
    
    # Get noise levels from first result
    if all_layer_results[0] and 'noise_perturbations' in all_layer_results[0]:
        noise_levels = list(all_layer_results[0]['noise_perturbations'].keys())
        for noise_std in noise_levels:
            averaged['noise_perturbations'][noise_std] = {'effect_size': 0.0}
    
    # Average across layers
    valid_results = [r for r in all_layer_results if r]
    num_layers = len(valid_results)
    
    for result in valid_results:
        if 'zero_perturbation' in result:
            averaged['zero_perturbation']['effect_size'] += result['zero_perturbation']['effect_size']
        if 'mean_perturbation' in result:
            averaged['mean_perturbation']['effect_size'] += result['mean_perturbation']['effect_size']
        if 'noise_perturbations' in result:
            for noise_std in noise_levels:
                if noise_std in result['noise_perturbations']:
                    averaged['noise_perturbations'][noise_std]['effect_size'] += result['noise_perturbations'][noise_std]['effect_size']
        if 'individual_effects' in result:
            for neuron in all_neurons:
                if neuron in result['individual_effects']:
                    if neuron not in averaged['individual_effects']:
                        averaged['individual_effects'][neuron] = {'effect_size': 0.0}
                    averaged['individual_effects'][neuron]['effect_size'] += result['individual_effects'][neuron]['effect_size']
    
    # Divide by number of layers
    averaged['zero_perturbation']['effect_size'] /= num_layers
    averaged['mean_perturbation']['effect_size'] /= num_layers
    for noise_std in noise_levels:
        averaged['noise_perturbations'][noise_std]['effect_size'] /= num_layers
    for neuron in all_neurons:
        if neuron in averaged['individual_effects']:
            averaged['individual_effects'][neuron]['effect_size'] /= num_layers
    
    return averaged


def analyze_your_neurons(neuron_indices=None, layer_idx=0, return_data_only=False):
    """Analyze the specific neurons you found."""

    if neuron_indices is None:
        neuron_indices = []
    print(f"Using top {len(neuron_indices)} neurons: {neuron_indices} (Layer {layer_idx})")
    ...
    analysis_results = analyzer.comprehensive_perturbation_analysis(texts, neuron_indices, layer_idx=layer_idx)
    ...
    info_metrics = analyzer.analyze_information_flow(texts, neuron_indices, layer_idx=layer_idx)
    print("Information flow metrics:", info_metrics)

    fig = analyzer.visualize_analysis(analysis_results, classification, layer_idx=layer_idx)
