...
def main():
    ...
    for model_name in args.models:
        ...
        layers = get_model_layers(model)
        if layers is None:
            print(f"⚠️ Could not find layers for {model_name}, using default layer {args.layer}")
            num_layers = 1
            layer_indices = [args.layer]
        else:
            num_layers = len(layers)
            layer_indices = list(range(num_layers))
            print(f"📊 Found {num_layers} layers, analyzing all layers: {layer_indices}")
        ...
        for layer_idx in layer_indices:
            ...
            all_results[model_name][layer_idx] = results

            # Note: Visualization is done after averaging across all layers (see below)
            try:
                ...
                conflict_results = analyzer.analyze_conflict_resolution(
                    conflict_sequences, high_var_neurons, layer_idx
                )
                results['analysis_results']['conflict_resolution'] = conflict_results
                # Don't visualize here - will be done after averaging
            except Exception as e:
            ...
            try:
                bias_top_neurons, mean_diff_per_neuron, bias_pairs, diff_matrix = compare_bias_detection_methods(
                    model_name=model_name, layer_idx=layer_idx, return_data_only=True
                )
                results['bias_neurons'] = bias_top_neurons
                if model_name not in bias_diff_matrices:
                    bias_diff_matrices[model_name] = {}
                bias_diff_matrices[model_name][layer_idx] = (diff_matrix, bias_pairs)
            except Exception as e:
                print(f"❌ Bias analysis failed for {model_name}: {e}")
                results['bias_neurons'] = []
                if model_name not in bias_diff_matrices:
                    bias_diff_matrices[model_name] = {}
                bias_diff_matrices[model_name][layer_idx] = (np.array([]), [])
    ...
    # Process each model and each layer - COLLECT DATA FIRST
    from bottleneck_analysis import average_bottleneck_results
    from bias_analysis import average_bias_results, plot_bias_heatmap
    from delta_intervention_analysis import plot_delta_intervention_averaged
    from polysemantic_analysis import plot_top_activations
    ...
    for model_name in args.models:
        ...
        all_bias_results = []
        all_suppression_changes = []
        all_amplification_changes = []
        all_conflict_results = []
        all_polysemantic_results = []
        
        for layer_idx in layer_indices:
            print(f"\n{'='*60}")
            print(f"🔬 Processing Layer {layer_idx}/{len(layer_indices)-1}")
            print(f"{'='*60}")
            
            # Get top neurons for this layer
            if model_name in all_results and layer_idx in all_results[model_name]:
                top_neurons = all_results[model_name][layer_idx].get('analysis_results', {}).get('delta_sensitive_neurons', [])
                if top_neurons:
                    top_neurons = [n for n, _ in top_neurons[:5]]
                else:
                    top_neurons = []
            else:
                # Fallback: compute neurons for this layer
                fallback_texts = load_analysis_texts(args.samples)
                delta_results, _ = load_or_compute_top_neurons(
                    model, tokenizer, fallback_texts, layer_idx, args.top_k, model_name
                )
                top_neurons = [n for n, _ in delta_results[:5]]

            # BOTTLENECK ANALYSIS - collect data
            print(f"\n🔎 BOTTLENECK NEURON ANALYSIS - Layer {layer_idx}")
            try:
                if len(top_neurons) > 0:
                    analysis_results, classification = analyze_your_neurons(top_neurons, layer_idx=layer_idx, return_data_only=True)
                else:
                    analysis_results, classification = analyze_your_neurons(layer_idx=layer_idx, return_data_only=True)
                all_bottleneck_results.append(analysis_results)
            except Exception as e:
                print(f"❌ Bottleneck analysis failed: {e}")

            # BIAS ANALYSIS - collect data
            try:
                bias_result = compare_bias_detection_methods(model_name=model_name, layer_idx=layer_idx, return_data_only=True)
                all_bias_results.append(bias_result)
            except Exception as e:
                print(f"❌ Bias analysis failed for {model_name}: {e}")

            # DELTA INTERVENTION - collect data
            print(f"\n📊 DELTA NEURON INTERVENTION ANALYSIS - Layer {layer_idx}")
            try:
                ...
                all_suppression_changes.append(suppression_changes)
                all_amplification_changes.append(amplification_changes)
            except Exception as e:
                print(f"❌ Delta intervention analysis failed: {e}")

            # STATE INTERPOLATION - collect data
            try:
                analyzer = StateInterpolationAnalyzer(model, tokenizer)
                conflict_sequences = analyzer.create_conflicting_sequences()
                conflict_results = analyzer.analyze_conflict_resolution(
                    conflict_sequences, top_neurons if top_neurons else [560], layer_idx
                )
                all_conflict_results.append(conflict_results)
            except Exception as e:
                print(f"❌ Conflict resolution analysis failed: {e}")

            # POLYSEMANTIC ANALYSIS - collect data
            try:
                polysemantic_result = run_polysemantic_analysis(neuron_index=560, layer_idx=layer_idx, return_data_only=True)
                all_polysemantic_results.append(polysemantic_result)
            except Exception as e:
                print(f"❌ Polysemantic analysis failed: {e}")

        # NOW AVERAGE AND PLOT ONCE
        print(f"\n{'='*60}")
        print(f"📊 AVERAGING DATA ACROSS ALL LAYERS AND GENERATING PLOTS")
        print(f"{'='*60}")
        
        # Average and plot bottleneck analysis
        if all_bottleneck_results:
            try:
                averaged_bottleneck = average_bottleneck_results(all_bottleneck_results)
                if averaged_bottleneck:
                    # Create a dummy classification for averaged results
                    avg_classification = {
                        'type': 'averaged',
                        'confidence': 1.0,
                        'metrics': {
                            'zero_effect': averaged_bottleneck['zero_perturbation']['effect_size'],
                            'noise_sensitivity': np.mean([v['effect_size'] for v in averaged_bottleneck['noise_perturbations'].values()]),
                            'mean_effect': averaged_bottleneck['mean_perturbation']['effect_size'],
                            'zero_to_noise_ratio': 0.0
                        }
                    }
                    analyzer = BottleneckAnalyzer(model, tokenizer)
                    analyzer.visualize_analysis(averaged_bottleneck, avg_classification, layer_idx=0, averaged=True)
            except Exception as e:
                print(f"❌ Failed to average bottleneck results: {e}")

        # Average and plot bias analysis
        if all_bias_results:
            try:
                bias_top_neurons, mean_diff_per_neuron, bias_pairs, diff_matrix = average_bias_results(all_bias_results)
                if diff_matrix is not None:
                    model_short_name = model_name.split('/')[-1]
                    plot_bias_heatmap(diff_matrix, bias_pairs, bias_top_neurons, 
                                     title=f"Bias Sensitivity Heatmap - {model_short_name}",
                                     layer_idx=0, averaged=True)
            except Exception as e:
                print(f"❌ Failed to average bias results: {e}")

        # Average and plot delta intervention
        if all_suppression_changes and all_amplification_changes:
            try:
                plot_delta_intervention_averaged(all_suppression_changes, all_amplification_changes)
            except Exception as e:
                print(f"❌ Failed to average delta intervention results: {e}")

        # Average and plot state interpolation
        if all_conflict_results:
            try:
                # Aggregate conflict results across layers
                aggregated_conflict = {
                    'conflict_types': {}
                }
                for conflict_result in all_conflict_results:
                    for conflict_type, data in conflict_result.get('conflict_types', {}).items():
                        if conflict_type not in aggregated_conflict['conflict_types']:
                            aggregated_conflict['conflict_types'][conflict_type] = {
                                'description': data.get('description', ''),
                                'summary': {
                                    'num_sequences': 0,
                                    'avg_smoothness': [],
                                    'avg_directional_consistency': [],
                                    'avg_trajectory_curvature': [],
                                    'resolution_patterns': {}
                                }
                            }
                        summary = data.get('summary', {})
                        agg_summary = aggregated_conflict['conflict_types'][conflict_type]['summary']
                        agg_summary['num_sequences'] += summary.get('num_sequences', 0)
                        if 'avg_smoothness' in summary:
                            agg_summary['avg_smoothness'].append(summary['avg_smoothness'])
                        if 'avg_directional_consistency' in summary:
                            agg_summary['avg_directional_consistency'].append(summary['avg_directional_consistency'])
                        if 'avg_trajectory_curvature' in summary:
                            agg_summary['avg_trajectory_curvature'].append(summary['avg_trajectory_curvature'])
                        if 'resolution_patterns' in summary:
                            for pattern, count in summary['resolution_patterns'].items():
                                agg_summary['resolution_patterns'][pattern] = agg_summary['resolution_patterns'].get(pattern, 0) + count
                
                # Average the metrics
                for conflict_type, data in aggregated_conflict['conflict_types'].items():
                    summary = data['summary']
                    if summary['avg_smoothness']:
                        summary['avg_smoothness'] = np.mean(summary['avg_smoothness'])
                    if summary['avg_directional_consistency']:
                        summary['avg_directional_consistency'] = np.mean(summary['avg_directional_consistency'])
                    if summary['avg_trajectory_curvature']:
                        summary['avg_trajectory_curvature'] = np.mean(summary['avg_trajectory_curvature'])
                
                analyzer = StateInterpolationAnalyzer(model, tokenizer)
                analyzer.visualize_state_interpolation(aggregated_conflict, [560], layer_idx=0, averaged=True)
            except Exception as e:
                print(f"❌ Failed to average state interpolation results: {e}")

        # Average and plot polysemantic analysis
        if all_polysemantic_results:
            try:
                # Average activation values per token across layers
                token_to_values = {}
                for result in all_polysemantic_results:
                    for val, token, word, context, full_text in result:
                        if token not in token_to_values:
                            token_to_values[token] = []
                        token_to_values[token].append(val)
                
                # Create averaged results
                averaged_results = []
                for token, values in token_to_values.items():
                    avg_val = np.mean(values)
                    # Use first layer's data for other fields
                    first_result = all_polysemantic_results[0]
                    matching = [r for r in first_result if r[1] == token]
                    if matching:
                        _, _, word, context, full_text = matching[0]
                        averaged_results.append((avg_val, token, word, context, full_text))
                
                # Sort by activation value
                averaged_results.sort(key=lambda x: x[0], reverse=True)
                plot_top_activations(averaged_results[:20], neuron_index=560, layer_idx=0, averaged=True)
                # Also generate clustering report for averaged results
                from polysemantic_analysis import cluster_contexts
                cluster_contexts(averaged_results[:20], neuron_index=560, layer_idx=None, num_clusters=3, averaged=True)
            except Exception as e:
                print(f"❌ Failed to average polysemantic results: {e}")
