...
def save_bias_heatmap_results(diff_matrix, bias_pairs, bias_top_neurons, model_name, layer_idx=0, averaged=False, cache_dir="bias_cache"):
    """
    Save bias heatmap results to a JSON file for offline use.
    
    Args:
        diff_matrix: numpy array of shape (num_pairs, num_neurons)
        bias_pairs: list of tuples (text1, text2)
        bias_top_neurons: list of tuples (neuron_idx, sensitivity)
        model_name: name of the model
        layer_idx: layer index
        averaged: whether results are averaged across layers
    ...
    Example:
        # Results are automatically saved when plotting, but you can also save manually:
        save_bias_heatmap_results(diff_matrix, bias_pairs, bias_top_neurons, 
                                  "gpt2", layer_idx=0)
    """
    ...
    if averaged:
        cache_path = os.path.join(cache_dir, f"bias_heatmap_{model_name_clean}_averaged.json")
    else:
        cache_path = os.path.join(cache_dir, f"bias_heatmap_{model_name_clean}_layer{layer_idx}.json")
    ...
    data = {
        ...
        "bias_top_neurons": bias_top_neurons,
        "num_pairs": len(bias_pairs),
        "num_neurons": len(bias_top_neurons),
        "matrix_shape": list(diff_matrix.shape)
    }
    
    with open(cache_path, 'w') as f:
        json.dump(data, f, indent=2)
    
    print(f"✅ Saved bias heatmap results to {cache_path}")
    return cache_path


def load_bias_heatmap_results(model_name, layer_idx=0, averaged=False, cache_dir="bias_cache"):
    """
    Load bias heatmap results from a saved JSON file.
    
    Args:
        model_name: name of the model
        layer_idx: layer index
        averaged: whether to load averaged results
        cache_dir: directory containing cache files
    
    Returns:
        dict with keys: diff_matrix, bias_pairs, bias_top_neurons, model_name, layer_idx, averaged
    ...
    # Convert diff_matrix back to numpy array
    data['diff_matrix'] = np.array(data['diff_matrix'])
    
    print(f"✅ Loaded bias heatmap results from {cache_path}")
    return data


def plot_bias_heatmap_from_cache(model_name, layer_idx=0, averaged=False, cache_dir="bias_cache"):
    """
    Load and plot bias heatmap from saved cache file.
    ...
    Args:
        ...
        cache_dir: directory containing cache files
    """
    data = load_bias_heatmap_results(model_name, layer_idx, averaged, cache_dir)
    if data is None:
        print(f"❌ Could not load bias heatmap results for {model_name}")
        return None
    
    # Extract data
    diff_matrix = data['diff_matrix']
    bias_pairs = data['bias_pairs']
    bias_top_neurons = data['bias_top_neurons']
    model_name_loaded = data['model_name']
    layer_idx_loaded = data['layer_idx']
    averaged_loaded = data['averaged']
    
    # Create title
    model_display_name = model_name_loaded.split('/')[-1]
    title = f"Bias Sensitivity Heatmap - {model_display_name}"
    
    # Plot
    plot_bias_heatmap(diff_matrix, bias_pairs, bias_top_neurons, 
                     title=title, 
                     layer_idx=layer_idx_loaded, 
                     averaged=averaged_loaded,
                     save_data=False)  # Don't save again since we loaded from cache
    
    return data


def plot_bias_heatmap(diff_matrix, bias_pairs, bias_top_neurons, title="Bias Sensitivity Heatmap", layer_idx=0, averaged=False, save_data=True):
    """
    ...
    plt.title(title + title_suffix, fontsize=20, pad=20, fontweight='bold')
    plt.xlabel("Neuron Index", fontsize=18, fontweight='bold')
    plt.ylabel("Bias Comparison", fontsize=18, fontweight='bold')
    
    # Set tick label font sizes explicitly on the axes
    # This ensures the font size is applied regardless of the number of labels
    ax.tick_params(axis='x', labelsize=16, rotation=45)
    ax.tick_params(axis='y', labelsize=16, rotation=0)
    
    # Also set font size directly on tick labels to ensure they're applied
    for label in ax.get_xticklabels():
        label.set_fontsize(16)
    for label in ax.get_yticklabels():
        label.set_fontsize(16)
    
    # Set colorbar label and tick label font sizes
    cbar = ax.collections[0].colorbar
    if cbar is not None:
        cbar.set_label('Bias Detection Score', fontsize=18, fontweight='bold')
    ...
    # Extract model name from title for saving
    if " - " in title:
        model_name_for_save = title.split(" - ")[-1].split(" Layer")[0].strip()
    else:
        # Try to extract from title in other formats
        model_name_for_save = title.replace("Bias Sensitivity Heatmap", "").strip().replace("-", "").strip()
        if not model_name_for_save:
            model_name_for_save = "model"
    
    # Clean model name for filename
    model_name_clean = model_name_for_save.replace('/', '_')
    
    if averaged:
        plt.savefig(f"images/bias_heatmap_{model_name_clean}.png", dpi=300, bbox_inches='tight')
        print(f"✅ Saved averaged bias heatmap to images/bias_heatmap_{model_name_clean}.png")
    else:
        plt.savefig(f"images/bias_heatmap_{model_name_clean}_layer{layer_idx}.png", dpi=300, bbox_inches='tight')
        print(f"✅ Saved bias heatmap to images/bias_heatmap_{model_name_clean}_layer{layer_idx}.png")
    plt.close()
    # plt.show()  # Not needed with non-interactive backend
    
    # Save data to file for offline use
    if save_data:
        save_bias_heatmap_results(diff_matrix, bias_pairs, bias_top_neurons, model_name_for_save, layer_idx, averaged)

# Updated main comparison function
def compare_bias_detection_methods(model_name="gpt2", layer_idx=0, return_data_only=False):
    """
    ...
    plot_bias_heatmap(diff_matrix, bias_pairs, bias_top_neurons, 
                     title=f"Bias Sensitivity Heatmap - {model_name.split('/')[-1]}",
                     layer_idx=layer_idx)
    
    return bias_top_neurons, mean_diff_per_neuron, bias_pairs, diff_matrix


def average_bias_results(all_layer_results: List[Tuple]) -> Tuple:
    """Average bias analysis results across all layers per neuron."""
    if not all_layer_results:
        return None, None, None, None
    
    # Extract all diff_matrices and bias_pairs
    all_diff_matrices = []
    bias_pairs = None
    
    for bias_top_neurons, mean_diff_per_neuron, bias_pairs_layer, diff_matrix in all_layer_results:
        if diff_matrix is not None and diff_matrix.size > 0:
            all_diff_matrices.append(diff_matrix)
            if bias_pairs is None:
                bias_pairs = bias_pairs_layer
    
    if not all_diff_matrices:
        return None, None, None, None
    
    # Average diff_matrix across layers
    averaged_diff_matrix = np.mean(all_diff_matrices, axis=0)
    
    # Average mean_diff_per_neuron
    all_mean_diffs = [mean_diff for _, mean_diff, _, _ in all_layer_results if mean_diff is not None]
    if all_mean_diffs:
        # Find max hidden size
        max_size = max(len(md) for md in all_mean_diffs)
        padded_diffs = []
        for md in all_mean_diffs:
            padded = np.zeros(max_size)
            padded[:len(md)] = md
            padded_diffs.append(padded)
        averaged_mean_diff = np.mean(padded_diffs, axis=0)
    else:
        averaged_mean_diff = None
    
    # Get top neurons from averaged mean_diff
    if averaged_mean_diff is not None:
        top_k = min(10, len(averaged_mean_diff))
        top_indices = np.argsort(averaged_mean_diff)[-top_k:][::-1]
        bias_top_neurons = [(int(i), float(averaged_mean_diff[i])) for i in top_indices]
    else:
        # Fallback: use first layer's top neurons
        bias_top_neurons = all_layer_results[0][0] if all_layer_results else []
    
    return bias_top_neurons, averaged_mean_diff, bias_pairs, averaged_diff_matrix
