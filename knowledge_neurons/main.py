...
def run_analysis_for_model(model_name, output_filename):
    ...
    total_layers = kn_finder.num_layers
    print(f"📊 Detected {total_layers} total layers in the model")
    
    # Use all layers instead of a subset
    layer_indices = list(range(total_layers))
    print(f"🔍 Will analyze all {len(layer_indices)} layers: {layer_indices}")
    ...
    # Run knowledge neuron analysis for all layers
    ...
    if is_mamba:
        ...
        try:
            ...
            # Run attention analysis using all layers
            attention_results = integrate_mamba_attention_neurons(
                model, sample_input, layer_indices=layer_indices, methods=['attention_weighted', 'gradient_guided']
            )
