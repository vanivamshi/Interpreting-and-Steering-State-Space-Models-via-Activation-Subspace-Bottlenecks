...
class MambaAttentionNeurons:
    ...
    def visualize_neurons(self, mamba_neurons: dict, layer_idx: int = 0, save_path: Optional[str] = None):
        ...
        Args:
            ...
            save_path: Optional path to save the visualization. If None, saves to figures/ folder
        """
        if layer_idx not in mamba_neurons:
        ...
        if save_path is None:
            save_path = os.path.join(figures_dir, f"mamba_neurons_layer_{layer_idx}.png")
        
        # If save_path is provided but not absolute, save to figures folder
