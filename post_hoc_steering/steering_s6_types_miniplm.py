...
Answer:''',
    ...
    logger.info(f"📋 Test suite contains {len(test_suite)} categories")
    ...
    for level_key, cases in test_suite.items():
        iteration_count += 1
        logger.info(f"🔄 Baseline iteration {iteration_count}/6: Processing {level_key} ({len(cases)} cases)")
        ...
        for i, case in enumerate(cases):
            ...
            try:
                ...
                logger.warning(f"   ⚠️  Interrupted at case {i+1}/{len(cases)}")
                raise
            except Exception as e:
                error_msg = str(e)
                logger.warning(f"   ⚠️  Error processing case {i+1}/{len(cases)}: {error_msg[:150]}")
                if response is None:
    ...
    logger.info(f"\n📊 Baseline test completed: {len(baseline_results)}/{len(test_suite)} categories processed")
