"""
SAFE mamba_deeptrace Context Fix - Logit Scaling Only
...
def load_mamba_deeptrace_with_safe_fix(mamba_deeptrace_weight=0.15, model_name="mamba_deeptrace-Fixed"):
    """
    Load mamba_deeptrace with safe context-length fix
    
    This version uses logit scaling which is 100% safe
    """
    # Import from your benchmark script
    try:
        from benchmark_optimized_weights import load_mamba_deeptrace
    except ImportError:
        try:
            from optimized_benchmark import load_mamba_deeptrace
        except ImportError:
            print("❌ Could not import load_mamba_deeptrace")
...
def patch_load_mamba_deeptrace():
    """
    Monkey-patch the load_mamba_deeptrace function to include scaling
    
    Usage in benchmark script:
        from mamba_deeptrace_safe_fix import patch_load_mamba_deeptrace
        patch_load_mamba_deeptrace()
        # Now load_mamba_deeptrace() will automatically include scaling
    """
    try:
        import benchmark_optimized_weights as bm
        
        # Store original
        original_load_mamba_deeptrace = bm.load_mamba_deeptrace
        
        def patched_load_mamba_deeptrace(mamba_deeptrace_weight=0.15, model_name="mamba_deeptrace"):
            # Call original
            model, tok = original_load_mamba_deeptrace(mamba_deeptrace_weight, model_name)
            
            # Add scaling if it's a mamba_deeptrace model
            if "mamba_deeptrace" in model_name.lower():
                add_context_aware_scaling(model)
            
            return model, tok
        
        # Replace
        bm.load_mamba_deeptrace = patched_load_mamba_deeptrace
        
        print("✓ Patched load_mamba_deeptrace() to include safe context scaling")
