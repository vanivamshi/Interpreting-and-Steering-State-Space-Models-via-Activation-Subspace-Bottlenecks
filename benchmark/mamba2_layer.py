# COMMENTED: Original mamba_deeptrace implementation with enhanced features
...
# ============ SIMPLIFIED FIXED mamba_deeptrace LAYER ============ #
class Fixedmamba_deeptraceLayer(nn.Module):
    """
    Simplified mamba_deeptrace layer with reduced complexity for better stability.
    Features:
    - Only 2 gates (input + forget) instead of multiple
    - Single SSM (not multiple timescales)
    - No problematic scaling parameters
    - Clean residual connections
    """
    def __init__(self, d_model, n_gates=2, d_state=16):
        super().__init__()
        self.d_model = d_model
        
        # Simplified gate mechanism - only 2 gates
        self.input_gate = nn.Linear(d_model, d_model)
        self.forget_gate = nn.Linear(d_model, d_model)
        
        # Single SSM (not multiple timescales)
        self.ssm = SSMBlock(d_model, d_state=d_state)
    ...
    def forward(self, x, layer_idx=0):
        ...
        x: [batch, seq, d_model]
        """
        # Input gating - select what to process
        input_gate = torch.sigmoid(self.input_gate(x))
        gated_input = x * input_gate
        
        # SSM processing
        ssm_out = self.ssm(gated_input, layer_idx)
        
        # Forget gating for residual connection
        forget_gate = torch.sigmoid(self.forget_gate(x))
        
        # Output with clean residual
        output = self.output_proj(self.norm(ssm_out))
        output = output * forget_gate + x * (1 - forget_gate)
        
        return output


# ============ UTILITY FUNCTIONS ============ #
def separation_loss(logits, targets, alpha=0.1):
    """
    Penalize when different classes have similar representations.
    Encourages class separation in the learned representations.
    
    Args:
        logits: [batch_size, num_classes] or [batch_size, d_model] feature representations
        targets: [batch_size] class indices
        alpha: Separation strength parameter
    ...
    for cls in unique_classes:
        ...
        if mask.sum() > 0:
            ...
            class_mean = logits[mask].mean(dim=0)
    ...
    distances = torch.cdist(class_means.unsqueeze(0), class_means.unsqueeze(0)).squeeze(0)
    
    # We want to maximize minimum distance between classes
    # Mask out self-distances (diagonal)
    mask = torch.eye(len(class_means), device=distances.device, dtype=torch.bool)
    ...
    # Loss encourages larger separation (smaller loss = larger distance)
    separation_loss_val = torch.exp(-min_distance * alpha)
    return separation_loss_val


def analyze_prediction_bias(model, test_loader, class_to_idx=None):
    """
    Figure out why model over-predicts certain classes.
    ...
    Returns:
        ...
        bias_report: Dictionary with bias analysis
    """
    model.eval()
    all_preds = []
    all_targets = []
    ...
    with torch.no_grad():
        for batch in test_loader:
            if isinstance(batch, dict):
                inputs = batch.get('input', batch.get('context', None))
                targets = batch.get('target', batch.get('answer', None))
            else:
    ...
    try:
        ...
        cm = sklearn_cm(all_targets, all_preds)
    except ImportError:
    ...
    # Check if specific classes are being over-predicted
    bias_report = {}
    if class_to_idx:
        for class_name, class_idx in class_to_idx.items():
            if class_idx < len(cm):
                # Count misclassifications as this class
                misclassifications_as = cm[:, class_idx].sum() - cm[class_idx, class_idx]
                # Count correct predictions
                correct_predictions = cm[class_idx, class_idx]
                # Count actual samples of this class
                total_samples = cm[class_idx, :].sum()
                
                bias_report[class_name] = {
                    'correct': correct_predictions,
                    'misclassified_as': misclassifications_as,
                    'total_samples': total_samples,
                    'accuracy': correct_predictions / total_samples if total_samples > 0 else 0.0
                }
                
                print(f"\n{class_name} (idx={class_idx}):")
                print(f"  Correct predictions: {correct_predictions}/{total_samples} ({bias_report[class_name]['accuracy']*100:.1f}%)")
                print(f"  Misclassified as {class_name}: {misclassifications_as}")
    
    return cm, bias_report


def create_simplified_mamba_deeptrace(d_model, d_state=16):
    """
    Minimal, verified mamba_deeptrace implementation.
    Creates a simplified sequential mamba_deeptrace module.
    ...
    Returns:
        Sequential module with mamba_deeptrace components
    """
    return nn.Sequential(
        ...
        # Single SSM block
        SSMBlock(d_model, d_state=d_state),
...
def initialize_from_mamba(enhanced_model, original_mamba):
    """
    Copy weights from working Mamba to mamba_deeptrace where possible.
    This helps initialize mamba_deeptrace with pre-trained Mamba weights.
    
    Args:
        enhanced_model: The mamba_deeptrace model to initialize
        original_mamba: The original Mamba model with weights
    
    Returns:
        enhanced_model with copied weights
    """
    enhanced_dict = enhanced_model.state_dict()
    original_dict = original_mamba.state_dict()
    
    # Copy matching parameters
    copied_count = 0
    for name, param in original_dict.items():
        if name in enhanced_dict:
            if enhanced_dict[name].shape == param.shape:
                enhanced_dict[name] = param.clone()
                copied_count += 1
                logger.info(f"Copied parameter: {name}")
            else:
                logger.warning(f"Shape mismatch for {name}: {enhanced_dict[name].shape} vs {param.shape}")
    
    enhanced_model.load_state_dict(enhanced_dict)
    logger.info(f"Copied {copied_count}/{len(enhanced_dict)} parameters from Mamba to mamba_deeptrace")
    return enhanced_model


# ============ DEBUGGING UTILITIES ============ #
def debug_gradient_flow(model, test_input):
    """
    Test gradient flow through the model.
    Returns True if gradients are flowing properly.
    """
    model.train()
    ...
    for name, param in model.named_parameters():
        if param.grad is not None:
            ...
            if grad_norm < 1e-7:
                zero_grad_count += 1
                print(f"❌ ZERO GRAD: {name} (norm: {grad_norm:.2e})")
        else:
            zero_grad_count += 1
            print(f"❌ NO GRAD: {name}")
    
    print(f"Total gradient norm: {total_grad_norm:.6f}")
    print(f"Parameters with zero/no gradients: {zero_grad_count}/{sum(1 for _ in model.named_parameters())}")
    
    return total_grad_norm > 1e-6  # Should be True


def verify_forward_pass(model, test_input):
    """
    Verify forward pass produces valid outputs.
    Checks for NaN, Inf, and reasonable statistics.
    """
    model.eval()
    with torch.no_grad():
        output = model(test_input)
        ...
        print(f"Input std: {test_input.std().item():.6f}")
        ...
        if has_nan:
            ...
            print(f"❌ NaN in output! Count: {nan_count}")
        else:
            print("✓ No NaN in output")
        
        if has_inf:
            inf_count = torch.isinf(output).sum().item()
            print(f"❌ Inf in output! Count: {inf_count}")
        else:
...
# COMMENTED: Enhanced mamba_deeptraceLayer implementation
# This implementation includes:
# - Enhanced SSMBlock with more parameters (5 gates, 32-state SSM)
# - Multiple scaling parameters for gradient monitoring
# - Pre/post normalization layers
...
# class Enhancedmamba_deeptraceLayer(nn.Module):
#     """
#     Enhanced mamba_deeptraceLayer with more parameters for increased influence.
#     """
#     def __init__(self, d_model: int, n_gates: int = 5, n_timescales: int = 3, d_state: int = 32):
    ...
    def forward(self, x: torch.Tensor, layer_idx: int = 0) -> torch.Tensor:
        ...
        memory_output = ssm_combined * self.attention_scale
...
def replace_layer_with_enhanced_mamba_deeptrace(model, layer_idx: int, d_model: int = 768):
    """
    FIXED: Properly replace layer with enhanced mamba_deeptrace
    """
    logger.info(f"Replacing layer {layer_idx} with FIXED mamba_deeptrace...")
    
    # Create final fixed mamba_deeptrace layer
    mamba_deeptrace_layer = mamba_deeptraceLayer(d_model=d_model, n_gates=3, n_timescales=3, d_state=16)
    ...
    mamba_deeptrace_layer.apply(init_weights)
    
    # Move to same device and dtype as the model
    mamba_deeptrace_layer = mamba_deeptrace_layer.to(device=device, dtype=dtype)
    
    # ✅ CRITICAL: Ensure all parameters are properly converted
    for name, param in mamba_deeptrace_layer.named_parameters():
        if param.dtype != dtype:
            param.data = param.data.to(dtype=dtype)
            logger.info(f"Converted parameter {name} from {param.dtype} to {dtype}")
    
    # Also ensure all buffers are converted
    for name, buffer in mamba_deeptrace_layer.named_buffers():
    ...
    # ✅ FIX: Create wrapper that ADDS mamba_deeptrace, doesn't replace
    class mamba_deeptraceWrappedLayer(nn.Module):
        def __init__(self, original, mamba_deeptrace, layer_idx):
            super().__init__()
            self.original = original
            self.mamba_deeptrace = mamba_deeptrace
            self.layer_idx = layer_idx
            self.norm = original.norm
            
            # ✅ ADD: Learnable mixing weight
            self.mix_weight = nn.Parameter(torch.tensor(0.1))  # Start with 10% mamba_deeptrace
        ...
        def forward(self, hidden_states, residual=None, inference_params=None, cache_params=None, **kwargs):
            ...
            mamba_deeptrace_out = self.mamba_deeptrace(hidden_states, self.layer_idx)
            
            if squeeze_output:
                mamba_deeptrace_out = mamba_deeptrace_out.squeeze(0)
            
            # ✅ FIX: Gradually mix in mamba_deeptrace
            mix_weight_sigmoid = torch.sigmoid(self.mix_weight)
            output = original_out * (1 - mix_weight_sigmoid) + mamba_deeptrace_out * mix_weight_sigmoid
            
            return output
    
    # Replace the layer
    wrapped = mamba_deeptraceWrappedLayer(original_layer, mamba_deeptrace_layer, layer_idx)
    model.backbone.layers[layer_idx] = wrapped
    
    logger.info(f"✅ Layer {layer_idx} now uses FIXED mamba_deeptrace with gradual mixing")
    return mamba_deeptrace_layer

def attach_mamba_deeptrace_layers(model: nn.Module, attribute_name: str = "mamba_deeptrace", use_fixed=True) -> int:
    """
    FIXED: Attach mamba_deeptrace layers with proper integration.
    
    Args:
        model: The model to attach mamba_deeptrace layers to
        attribute_name: Name of the attribute to store mamba_deeptrace layer
        use_fixed: If True, use simplified Fixedmamba_deeptraceLayer (recommended)
    """
    if use_fixed:
        logger.info("Attaching simplified Fixedmamba_deeptrace layers...")
    else:
        logger.info("Attaching enhanced mamba_deeptrace layers (using working implementation)...")
    ...
    for idx, layer in enumerate(layers):
        ...
        # Use Fixedmamba_deeptraceLayer if requested, otherwise use original complex version
        if use_fixed:
            # Simplified Fixedmamba_deeptraceLayer
            mamba_deeptrace_layer = Fixedmamba_deeptraceLayer(d_model=d_model, n_gates=2, d_state=16)
        else:
            # Original complex mamba_deeptraceLayer (fallback)
            try:
                # Try enhanced attachment for first few layers
                if idx < 3:
                    mamba_deeptrace_layer = replace_layer_with_enhanced_mamba_deeptrace(model, idx, d_model)
                    count += 1
                    logger.info(f"✅ Enhanced mamba_deeptrace attached to layer {idx}")
                    continue
            except Exception as e:
                logger.warning(f"Failed to attach enhanced mamba_deeptrace to layer {idx}: {e}")
            
            # Fallback to simple mamba_deeptraceLayer
            mamba_deeptrace_layer = mamba_deeptraceLayer(d_model)
        ...
        setattr(layer, attribute_name, mamba_deeptrace_layer)
        count += 1
            
    logger.info(f"✅ Attached mamba_deeptrace layers to {count} layers")
    return count


def test_mamba_deeptrace_gradient_flow():
    """Test that gradients actually flow through mamba_deeptrace"""
    mamba_deeptrace = mamba_deeptraceLayer(d_model=768, n_gates=3, d_state=16)
    
    x = torch.randn(2, 10, 768, requires_grad=True)
    output = mamba_deeptrace(x)
    ...
    for name, param in mamba_deeptrace.named_parameters():
        if param.grad is not None:
    ...
    for param_name in key_params:
        param = dict(mamba_deeptrace.named_parameters())[param_name]
...
# ============ TEST FIXED mamba_deeptrace LAYER ============ #
def test_fixed_mamba_deeptrace_layer():
    """
    Test the simplified Fixedmamba_deeptraceLayer with gradient flow and forward pass verification.
    """
    print("="*60)
    print("Testing Fixedmamba_deeptraceLayer")
    print("="*60)
    ...
    # Create layer (n_gates parameter kept for compatibility, but not used in new design)
    layer = Fixedmamba_deeptraceLayer(d_model=d_model, n_gates=2, d_state=16)
    print(f"✓ Created Fixedmamba_deeptraceLayer with d_model={d_model}, d_state=16")
    ...
    is_valid = verify_forward_pass(layer, test_input)
    if is_valid:
    ...
    print("\n--- Gradient Flow Test ---")
    test_input = torch.randn(batch_size, seq_len, d_model, requires_grad=True)
    has_grads = debug_gradient_flow(layer, test_input)
    if has_grads:
    ...
    print("\n" + "="*60)
    print("✓ All tests passed for Fixedmamba_deeptraceLayer!")
    print("="*60)
    return True


def identify_mamba_deeptrace_circuits(self, layer_idx: int = 0) -> Dict[str, Any]:
    """
    Identify circuit candidates specifically for mamba_deeptrace architecture.
    
    mamba_deeptrace has distinct components that form circuits:
    ...
    """
    logger.info(f"Step 6a: Identifying mamba_deeptrace-specific circuits for layer {layer_idx}...")
    
    if not hasattr(self, 'mamba_deeptrace_activation_data') or layer_idx not in self.mamba_deeptrace_activation_data:
        logger.warning(f"No mamba_deeptrace activation data found for layer {layer_idx}")
        return {}
    
    activation_data = self.mamba_deeptrace_activation_data[layer_idx]
    circuits = []
    ...
    # Save Results
    # ========================================
    circuits_file = self.experiment_logger.experiment_dir / f"mamba_deeptrace_candidate_circuits_layer_{layer_idx}.json"
    
    result = {
        'circuits': circuits,
        'layer_idx': layer_idx,
        'architecture': 'mamba_deeptrace',
    ...
    logger.info(f"✓ Identified {len(circuits)} mamba_deeptrace circuits for layer {layer_idx}")
    logger.info(f"  Circuit types: {', '.join(result['circuit_types'])}")
    
    return result


def add_mamba_deeptrace_activation_hooks(analyzer, model, layer_idx: int = 0):
    """
    Add hooks to capture mamba_deeptrace-specific activations for circuit analysis.
    Must be called before running forward passes.
    """
    if not hasattr(analyzer, 'mamba_deeptrace_activation_data'):
        analyzer.mamba_deeptrace_activation_data = {}
    
    analyzer.mamba_deeptrace_activation_data[layer_idx] = {}
    ...
    if not hasattr(layer, 'mamba_deeptrace'):
        logger.warning(f"Layer {layer_idx} does not have mamba_deeptrace module")
        return
    
    mamba_deeptrace = layer.mamba_deeptrace
    
    # Hook for each gate in ensemble
    for i, gate in enumerate(mamba_deeptrace.gates):
        def gate_hook(module, input, output, gate_idx=i):
            analyzer.mamba_deeptrace_activation_data[layer_idx][f'gate_{gate_idx}'] = output.detach().cpu().numpy()
        gate.register_forward_hook(gate_hook)
    
    # Hook for each timescale SSM
    for name in ['ssm_fast', 'ssm_medium', 'ssm_slow']:
        ssm = getattr(mamba_deeptrace, name)
        def ssm_hook(module, input, output, ssm_name=name):
            analyzer.mamba_deeptrace_activation_data[layer_idx][ssm_name] = output.detach().cpu().numpy()
        ssm.register_forward_hook(ssm_hook)
    
    # Hook for sparse attention (if applicable)
    def attn_hook(module, input, output):
        analyzer.mamba_deeptrace_activation_data[layer_idx]['sparse_attn'] = output.detach().cpu().numpy()
    mamba_deeptrace.sparse_attn.register_forward_hook(attn_hook)
    
    # Hook for memory gate
    def mem_gate_hook(module, input, output):
        analyzer.mamba_deeptrace_activation_data[layer_idx]['memory_gate'] = output.detach().cpu().numpy()
    mamba_deeptrace.memory_gate.register_forward_hook(mem_gate_hook)
    
    # Hook for compression predictor
    def comp_hook(module, input, output):
        analyzer.mamba_deeptrace_activation_data[layer_idx]['compression'] = output.detach().cpu().numpy()
    mamba_deeptrace.compression_predictor.register_forward_hook(comp_hook)
    
    logger.info(f"✓ Added mamba_deeptrace activation hooks for layer {layer_idx}")


# ============================================================================
# FINAL FIXED mamba_deeptrace Implementation - All gradient flow issues resolved
# ============================================================================
...
class mamba_deeptraceLayer(nn.Module):
    """
    FINAL FIXED mamba_deeptrace Layer with all gradient issues resolved
    """
...
def replace_layer_with_final_fixed_mamba_deeptrace(model, layer_idx: int, d_model: int = 768):
    """
    FINAL FIXED: Replace layer with properly integrated mamba_deeptrace
    """
    import logging
    logger = logging.getLogger(__name__)
    
    logger.info(f"Replacing layer {layer_idx} with FINAL FIXED mamba_deeptrace...")
    
    # Create fixed mamba_deeptrace layer
    mamba_deeptrace_layer = mamba_deeptraceLayer(d_model=d_model, n_gates=3, n_timescales=3, d_state=16)
    mamba_deeptrace_layer = mamba_deeptrace_layer.to(next(model.parameters()).device)
    mamba_deeptrace_layer = mamba_deeptrace_layer.to(next(model.parameters()).dtype)
    ...
    class mamba_deeptraceWrappedLayer(nn.Module):
        def __init__(self, original, mamba_deeptrace, layer_idx):
            ...
            # ✅ FIXED: Stable mixing weight (start at 50% mamba_deeptrace)
        ...
        def forward(self, hidden_states, residual=None, inference_params=None, cache_params=None, cache_position=None, **kwargs):
            ...
            # mamba_deeptrace output
            if hidden_states.dim() == 2:
                hidden_states_3d = hidden_states.unsqueeze(0)
            else:
                hidden_states_3d = hidden_states
            
            mamba_deeptrace_out = self.mamba_deeptrace(hidden_states_3d, self.layer_idx)
            ...
            output = original_out * (1 - mix_alpha) + mamba_deeptrace_out * mix_alpha
    ...
    logger.info(f"✅ Layer {layer_idx} now uses FINAL FIXED mamba_deeptrace")
...
if __name__ == "__main__":
    # Test Fixedmamba_deeptraceLayer
    print("\n=== Testing Fixedmamba_deeptraceLayer ===\n")
    test_fixed_mamba_deeptrace_layer()
    
    print("\n=== Testing original mamba_deeptraceLayer ===\n")
    # Test gradient flow for original implementation
    print("Testing gradient flow...")
    test_mamba_deeptrace_gradient_flow()
    
    # Minimal sanity test when run directly
    torch.manual_seed(0)
    d_model = 64
    layer = mamba_deeptraceLayer(d_model)
